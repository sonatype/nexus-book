...
After you complete the database export and store the files to a secure location, you can <<backup-restore
...
the location you specified in the `Backup location` field.

In preparation for the file restore, you should only remove the database directories you want to restore (i.e. from
`$data-dir/nexus3/db`). The restoration will not overwrite the existing data of database left in place.
...
4. Restart {nxrm}.
5. View the console logs for `database config restored`, which is logged at the `INFO` level.
