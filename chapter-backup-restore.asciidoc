...
While the backup process is running, Nexus Repository Manager will automatically place the database in read-only mode.
This is done for both single node and HA deployments to preserve data integrity. While in read-only mode, attempts to
publish new components or make changes to repository manager configuration will fail.

Nexus Repository Manager will automatically release the read-only status when backup completes. Schedule the backup task
