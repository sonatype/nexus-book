...
To restore the databases from backups, follow the following steps.

1. If nexus won't start, install a fresh copy of nexus using the same version that backups were created
from.
2. Ensure nexus is stopped.
3. Create a folder under sonatype-work/nexus3 called backup.
4. Copy the .bak files to be restored into this folder.
5. Remove the existing database files from sonatype-work/nexus3/db if they exist.  The database restorer
 won't overwrite any existing data.
6. Restart nexus

To confirm the restore was triggered, check the logs for a message like "database config restored", which
will be logged at the INFO level.

If the component database is restored, the corresponding blobstores that hold the components must also be
restored.

Notes for HA:

The restoration process should be run on a single node.  All nodes in an HA cluster should be stopped prior to
restoration.
