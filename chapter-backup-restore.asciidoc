...
1. If {nxrm} won't start, install a fresh copy of the repository manager on a different server; it must be the
same version where the `.bak` files were created.
2. Go to the location  `$data-dir/nexus3/backup`.
3. Copy the `.bak` files you wish to restore to `$data-dir/nexus3/backup`.
5. Remove any existing subfolders from `$data-dir/nexus3/db` if they exist. The restoration won't overwrite any
existing data.
