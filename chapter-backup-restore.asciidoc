...
and fully recover the exported content of {nxrm}.  
...
It is important to use the 'Export configuration & metadata for backup' task because it generates a snapshot 
of the databases preserved for backup instead of copying the entire data directory, where they reside. 
Exporting the data directory without configuring the task and copying the output to another location may reflect 
a state of the directory that's inconsistent with the output you can automatically schedule with 
