...
While the backup process is running, {nxrm} will automatically place the database in read-only mode.
This is done for both single node and HA deployments to preserve data integrity. While in read-only mode, attempts to
publish new components or make changes to {nxrm} configuration will fail.
