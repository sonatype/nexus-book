...
You can configure the repository manager to run as a service with +systemd+. +systemd+ is a startup framework 
used in Linux-based systems such as Ubuntu and CentOS. It is, essentially, an initscript that loads commands to 
manage the repository manager daemon. 
...
This example script uses +systemd+ to run the repository manager service. Create a file called +nexus.service+. 
Add the following contents then save.
