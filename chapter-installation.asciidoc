...
==== Configuring an External Runtime Environment

As mentioned in <<installation-java>> you have to install either the Java Development Kit or Java Runtime 
Environment to use {pro}. You can install the JDK which includes the JRE, or install the JRE by itself. 
Whichever method you choose, be sure the variable path is directed to the corrct user running the repository 
manager or service.

To set the path for a specific JRE location open the +nexus+ script and locate 
the line +INSTALL4J_JAVA_HOME_OVERRIDE+. Remove the hash and corresponding 
...
Applying the JDK or JRE to +INSTALL4J_JAVA_HOME_OVERRIDE+ variable is advantageous because it bypasses the JDK. 
Therefore it can run without affecting other programs.
