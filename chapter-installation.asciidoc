...
Nexus is essentially a sophisticated web application, answering HTTP requests
using the high-performance servlet container http://eclipse.org/jetty[Eclipse
Jetty]. Jetty is very capable and highly configurable. Using a
https://en.wikipedia.org/wiki/Reverse_proxy[reverse proxy] in front of Nexus
is not typically required or necessary.

In some cases organizations are required to run applications behind a reverse
proxy for security concerns, network administrator familiarity, company policy
or to consolidate multiple disparate applications and virtual hosts.
...
[[nexus_web_application_context_path]]
==== Nexus Webapp Context Path

Nexus webapp context path is by default +/nexus+. This means every URL path used
...
At Administration -> Server -> Application Server Settings Nexus has a
...
proxy server and <<nexus_web_application_context_path,edit the Nexus webapp
path to be slash ( +/+ )>>.
