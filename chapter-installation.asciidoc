...
by the repository manager. The default location of the data directory is `../sonatype-work/nexus3` relative to the
nexus installation directory. In this section, and throughout the book, it is referred to as `$data-dir` in any
code segments.
...
the ports used for HTTP and HTTPS access, as well as the context path and host. You can override these
defaults in `$data-dir/etc/nexus.properties`
...
`$data-dir/etc/nexus.properties` with
...
The Legacy URL Paths have the exact same behaviour for client tools as {nxrm} 2, allowing you to download as well
as deploy components with the same API/HTTP calls. However, accessing the legacy URLs in the browser as a user
does not support the directory browsing mode for inspecting the content of a repository as supported in
{nxrm} 2. The browsing user interface for components and assets as documented in <<browse-browse>> can be
used intead.
