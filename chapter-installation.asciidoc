...
repo.somecompany.com.

Let's assume that you wanted to host Nexus behind Apache httpd at the
URL +http://repo.somecompany.com+. To do this, you'll need to change the
...
. Set the Base URL in Nexus as shown in
  ...
  URL of Nexus e.g. +http://repo.somecompany.com+ 

At this point, edit the httpd configuration file for the
repo.somecompany.com virtual host. Include the following to expose
Nexus via mod_proxy at http://repo.somecompany.com/.
...
If you want to serve Nexus on a context path or port that is different than the one it
...
The http://nginx.org/en/docs/[nginx documentation] contains furhter
