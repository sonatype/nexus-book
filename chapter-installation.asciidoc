...
Add the following contents, then save the file in your +usr/local/bin+ directory.
...
{pro} is a sophisticated server application with a web-application user interface, answering 
...
This section provides some general guidance on how to configure common reverse proxy servers to work with {pro} 
and {oss}. Always consult your reverse proxy administrator to ensure you configuration is secure.
...
In the following examples, refer to <<config-http-port>> to change HTTP port and context properly.
