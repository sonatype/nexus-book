...
<<<<<<< HEAD
=======
The Nexus application directory +nexus-3.0.0-b2015091801+ requires another directory named +sonatype-work+. This
directory contains all of the repository and configuration data for Nexus and is stored outside of the Nexus
installation directory to make it easier to upgrade to a newer version of Nexus.

By default, this directory is a sibling to the Nexus installation directory. If you installed Nexus in the +/opt+
directory it would also contain a +sonatype-work+ subdirectory with a nested +nexus+ directory containing all of the
...
existing repository configuration from the +TBD+ directory.
...
defaulting to +tbd/nexus+, to the +nexus+ user that will run
the application.
...
+TBD/nexus+. When you upgrade to a newer version of Nexus, you
replace the Nexus application directory and retain all of your own
custom configuration and repository data in +TBD/+.

[[sect-installing-work-dir]]
==== Nexus Data Directory

The Nexus `data` directory +TBD+ is created as a sibling to
...
The Sonatype Work Nexus directory +TBD/nexus/+ contains a
...
<<<<<<< HEAD
this file. If you wanted to customize the location of the +TBD+ 
=======
...
Nexus logs events in the +TBD/logs/nexus.log+ file. In
