...
the 'installation directory'.  After you start the application, a sibling folder appears with related resources.
This is the 'data directory', which is used for component and repository storage. Further details about these folders can be found in <<directories>>.
...
managed by the repository manager. This is a sibling to the installation directory and contains archive-based installs. The default location of the data directory is `sonatype-work/nexus3/data`. In this documentation it is referred to as `$data-dir` in any code segments.
...
The data directory contains subdirectories such as `blobs`, `db`, `elasticsearch`, and others. These contain all the components, repository, configuration, and other data presented by the repository manager. Follow the instructions in <<config-data-directory>> to customize the location.
...
-Dkaraf.data=../sonatype-work/nexus3/data
-Djava.io.tmpdir=../sonatype-work/nexus3/data/tmp
...
You can use `$install-dir/bin/nexus.vmoptions` to define a new location for data you want to preserve. In the configuration file, change the values of `Dkaraf.data` and `Djava.io.tmpdir` to designate an absolute path you prefer to use.

For example, to use the absolute path `/opt/repository/storage` change the values as follows:
