...
The data directory, found by default at `../sonatype-work/nexus3`, contains subdirectories such as `blobs`, `db`,
`elasticsearch`, and others. These contain all the components, repository, configuration, and other data 
presented by the repository manager. Follow the instructions in <<config-data-directory>> to customize the 
location.
