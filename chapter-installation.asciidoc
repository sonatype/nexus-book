...
==== Configuring the Java Runtime Environment Path for Linux and Unix

Linux and Unix do not come with Java Development Kit nor Java Runtime Environment (JRE). You can install the JDK 
which includes the JRE or, install the JRE by itself. Whichever method you choose, update your +JAVA_HOME+ or 
+JDK_HOME+ in your +.bashrc+ to the path that runs the service.
...
To set the path for a specific JRE location open the +nexus+ script and locate 
line+INSTALL4J_JAVA_HOME_OVERRIDE+. Remove the hash and corresponding 
comment, then add the JRE path to the unhidden variable, like the example below.

----
INSTALL4J_JAVA_HOME_OVERRIDE=/opt/nexus/java/jre1.8.0_77
---- 

Applying the JRE to +INSTALL4J_JAVA_HOME_OVERRIDE+ variable is advantageous because it bypasses the bundled 
runtime package in the JDK. Therefore you can upgrade the JRE without affecting other programs.
...
Copy +nexus+ script to the +etc/init.d+ directory.
