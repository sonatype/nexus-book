...
==== Configuring Java Runtime Environment for Linux and Unix

Linux and Unix comes with OpenJDK, an open source implementation of the Java Platform. Since the repository 
manager does not support OpenJDK you are required to install Java Runtime Environment (JRE) from Oracle, then 
manually set your environment variables to point to the Java directory on your system. If you download the Java 
Development Kit (JDK) bundle, it includes the JRE.

After you download and install the preferred Java platform set the appropriate variables on your +.bashrc+. This 
example shows configuation for +JAVA_HOME+, +JRE_HOME+, and +PATH+.
