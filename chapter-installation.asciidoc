...
Pro].  Nexus OSS is a fully-featured repository
...
Nexus OSS can also be deployed as a web application in a
...
----
$ unzip nexus-2.9.0-02-bundle.zip
----

And, if you download the GZip'd TAR archive, run:

----
$ tar xvzf nexus-2.9.0-02-bundle.tar.gz
----

For Nexus professional the equivalent commands would be 

----
$ unzip nexus-professional-2.9.0-02-bundle.zip
$ tar xvzf nexus-professional-2.9.0-02-bundle.tar.gz
----
...
assumes that Nexus is installed in +/usr/local/nexus-2.9.0-02+
...
The Nexus OSS WAR can run on most Java application servers.
To download the Nexus OSS WAR, go to
...
for Nexus Pro is only provided for the bundle install.
...
+nexus-2.9.0-02.war+ file in +$TOMCAT_HOME/webapps+ and restart your
Tomcat instance. Assuming that Tomcat is configured on port 8080 once
Tomcat is started, Nexus will be available on
http://localhost:8080/nexus-2.9.0-02[http://localhost:8080/nexus-2.9.0-02].

If you would like a less verbose URL, copy +nexus-2.9.0-02.war+ to a
