...
{nexus} only has one prerequisite, a Java Runtime Environment
...
application with an Eclipse Jetty server and supplementary files like
startup scripts.
...
hard coded directories. It will run from any directory. If you
downloaded the ZIP archive, run:
...
directory other than your home directory. On a Unix machine, we assume
that Nexus is installed in +/opt1+ with a symbolic link +/opt/nexus+
to the versioned +nexus+ directory. Using a generic symbolic link
+nexus+ to a specific version is a common practice which makes it
easier to upgrade when a newer version of Nexus is made available.
...
The Nexus installation directory +nexus-3.0.0-b2015061001+ or
+nexus-professional-3.0.0-b2015061001+ has a sibling directory named
...
----
INFO  [jetty-main-1] *SYSTEM org.eclipse.jetty.server.Server - Started @36628ms
INFO  [jetty-main-1] *SYSTEM org.sonatype.nexus.bootstrap.jetty.JettyServer - Started
----

At this point, Nexus will be running and listening on all IP addresses
that are configured for the current host on port 8081. To
