...
so that no username information is transmitted. The 'Anonymzation
...
If you have selected to automatically submit the analytics data, a
scheduled task was created named 'Automatically submit analytics
events'. This task is preconfigured to run at 1am every day. If
desired the recurrence can be changed in the scheduled tasks
administration area documented in <<scheduled-tasks>>.

IMPORTANT: Sonatype values your input greatly and hopes you will
activate the analytics feature and the automatic submission to allows
