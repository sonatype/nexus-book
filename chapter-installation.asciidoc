...
the 'installation directory'.  After you start the application, a sibling folder appears with related resources. This is the 'data directory', which is used for component and repository storage. Further details about these folders can be found in <<directories>>.
...
managed by the repository manager. This is sibling to the installation directory by default for archive-based installs.It is located at `sonatype-work/nexus3/data`. In this documentation it is referred to as `$data-dir` in any code segments.
