...
simple process. This chapter provides further details for the get
...
{oss} and {pro} only have one prerequisite, a Java Runtime Environment
(JRE) compatible with Java 7 or Java 8 from Oracle. Nexus is most
often run with the JRE that is bundled with a Java Development Kit
(JDK) installation. We recommend to use the latest version of Java
available from the
...
the correct version is using by running the above command as the
...
The following instructions are for installing {oss} or {pro} as a
stand-alone server. The Nexus bundle combines the Nexus web
application with a Eclipse Jetty server and supplementary files like
startup scripts.

Installing Nexus is straightforward. Unpack the Nexus bundle archive
in a directory. If you are installing Nexus on a local workstation to
give it a test run, you can install it in your home directory or
wherever you like. Nexus doesn't have any hard coded directories. It
will run from any directory. If you downloaded the ZIP
...
assumes that Nexus is installed in +/opt/nexus3.0.0-b2015061001+
with a symbolic link +/opt/nexus+ to the +nexus+ directory. Using a
...
----
$ sudo cp nexus-2.9.0-02-bundle.tar.gz /usr/local
$ cd /usr/local
$ sudo tar xvzf nexus-2.9.0-02-bundle.tar.gz
...
The Nexus installation directory +nexus-2.9.0-02+ or
+nexus-professional-2.9.0-02+ has a sibling directory named
...
Eclipse Jetty. Nexus ships with generic
...
http://localhost:8081/[http://localhost:8081/nexus]. You should see 
