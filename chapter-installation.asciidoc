...
NOTE: The installer includes a feature you can configure to run the repository manager as a service. To run on a
Linux operating machine, see <<service-linux>>. To run on Mac OS X, see <<service-osx>>. To run on Windows, see 
<<service-windows>>.
...
The service is available in common console applications such as Windows Services. Open up Windows Services and 
locate the program called 'nexus', then start the service in the repository manager.
...
To uninstall {oss} from your system locate the +uninstall+ file in the installation directory. 
...
Click 'Next' to complete the uninstall process.
