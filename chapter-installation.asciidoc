...
+nexus-2.11.0-02-bundle.zip+ or +nexus-2.11.0-02-bundle.tar.gz+.
...
----
$ unzip nexus-2.11.0-02-bundle.zip
----

And, if you download the GZip'd TAR archive, run:

----
$ tar xvzf nexus-2.11.0-02-bundle.tar.gz
----

For Nexus professional the equivalent commands would be 

----
$ unzip nexus-professional-2.11.0-02-bundle.zip
$ tar xvzf nexus-professional-2.11.0-02-bundle.tar.gz
----
...
assumes that Nexus is installed in +/usr/local/nexus-2.11.0-02+
...
----
$ sudo cp nexus-2.11.0-02-bundle.tar.gz /usr/local
$ cd /usr/local
$ sudo tar xvzf nexus-2.11.0-02-bundle.tar.gz
$ sudo ln -s nexus-2.11.0-02 nexus
----
...
The Nexus installation directory +nexus-2.11.0-02+ or
+nexus-professional-2.11.0-02+ has a sibling directory named
...
WARNING: We recommend to avoid running Nexus as the root user or a similar 
privileged user, as this practice poses serious security risk to the host 
operating system unnecessarily. Instead we suggest to follow system administration
best practice and use a service specific user with the minimum requires
access rights only.
...
+nexus-2.11.0-02.war+ file in +$TOMCAT_HOME/webapps+ and restart your
Tomcat instance. Assuming that Tomcat is configured on port 8080 once
Tomcat is started, Nexus will be available on
http://localhost:8080/nexus-2.11.0-02[http://localhost:8080/nexus-2.11.0-02].

If you would like a less verbose URL, copy +nexus-2.11.0-02.war+ to a
