...
`health-check/` :: This file contains cached reports from the {rhc} feature.
...
`blobs/` :: This file contains a subdirectory of blob stores configured from the user interface. A +default+ blob
store directory is included with the repository manager. Each time you create a new blob store from the 'Repositories'
menu, in 'Administration', the new directory appears as a sibling to +default+. The blob store files contain
repository components and their assets.

`etc/` :: This directory contains the main runtime configuration and customization of the repository manager. The files
are explained further in <<configure-runtime>>.

`instances` :: This file contains information on all running instances of {nxrm}.

`cache` :: This file contains information on currently cached Karaf bundles.

`keystores` :: This contains the automatically generated key used to identify your repository manager.

`db` :: This file contains the Orient databases that store the repository manager's underlying component and metadata.

`tmp` :: This file is used for temporary storage.
...
with snapshot of the databases, in the data directory. See <<backup-preparation>> to learn how to configure this task.
