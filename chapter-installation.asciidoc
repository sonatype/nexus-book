...
upgrade an existing installation. You can upgrade with the installer application or the tar.gz or zip 
distribution file. To keep the upgrade simple be sure to schedule some downtime.

NOTE: Upgrading to {oss} 3.0.0 can only be performed by users who run the milestone 7 release of the repository 
manager. Be sure to manually back up the milestone 7 data dictionary to another location. It is a crucial step to 
properly upgrade the application.

==== Upgrade with the Installer Application

To upgrade using the installer, locate the data dictionary of your previous installation, then stop the 
repository manager. Start the installer after downloading. It will automatically detect the existing install, 
then ask if you want to upgrade. Select the radio button 'No, install into a different directory' and click 
'Next>'.

Continue until you see the screen that displays the 'Destination Directory' field. Enter a path to a directory 
where the repository manager application runtime files will reside.

Click 'Next>' to get the screen that displays a field for 'Data Dictionary'. Enter a path to the previous
data dictionary directory. Inside the target directory, create an empty file called +clean_cache+. Click 'Next>'.

The last screen of the installer will prompt you before it overwrites any files. Choose 'Yes' or 'Yes to all' to 
overwrite existing files.

==== Upgrade with a Distribution File

To upgrade using a +tar.gz+ or +zip+ file, stop the repository manager, if running. Then download the 
distribution file applicable to your operating system. Extract the appropriate file.
...
Edit +nexus-3.0.0-03/bin/nexus.vmoptions+. Replace the line +-Dkaraf.data=data+ to point to the previous 
data directory. This example shows the path +/app/nexus3/data+:

----
-Dkaraf.data=/app/nexus3/data
----

Inside the previous data dictionary directory, create an empty file called +clean_cache+, e.g. 
+/app/nexus3/data/clean_cache+

NOTE: To see a full example of upgrading, printing contents of the data dictionary, and more review the 
