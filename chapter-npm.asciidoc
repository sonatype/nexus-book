...
file does not exist simply create it and add the registry configuration with the URL of your npm repository
group. You can see the public URL of your group repository in the repository list in the 'Repository Path' column.
...
output will reference the URLs:
...
Publishing requires authentication. It can be configured for your user account using +npm login+ on the command
line.  Provide your repository manager username and password when prompted.  Upon successful completion, a line
for authentication of this combination is automatically added to your +.npmrc+ configuration file.  Nexus supports
