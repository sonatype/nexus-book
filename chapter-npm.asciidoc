...
Once you set up your hosted and proxy repositories for npm packages and create a repository group to merge them, you
can access them with the +npm+ tool on the command line as one registry.
...
repository. Scoped packages are distinguishable with a `@` placed in front of the scope, followed by a
slash (`/`) and package name (e.g. `@scope/project-name`). To publish:
...
When you publish the package a folder is created with a `@` preceding name of the scope. In the repository
manager, you can browse scoped packages from the 'Repository Path' column. These packages are alphabetically
indexed, in tree view, amid your hosted repository's overall list of packages.
