...
repository group via 'Artifact Search'.

Scoped packages are distinguishable with a `@` placed in front of the `npm` module name, followed by a slash
(e.g. `@scope/project-name`). When published the `@` before each package is sorted at top of the repository's
indexed list preceding the non-scoped packages in alphabetical order. Further details on scoping are available
on the https://docs.npmjs.com/misc/scope[npm website].
