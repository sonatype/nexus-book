...
If you cannot use the realm and login method, for example if you have a username which includes capital letters
(disallowed by npm login), you can configure npm to use basic auth with your repository manager. This 
authentication method involves editing the +.npmrc+ configuration file adding an encoded username and password as 
...
TIP: Assure your file does not have extra whitespace or a trailing line seperator as either of these will 
...
Once you have the encoded credentials, it as well as the author email and enabling of authentication can then be 
added to the +.npmrc+ file (below your already entered registry config).  For example, for default admin:
