...
systems or as manually run deployment. Once Nexus hosts the
...
including e.g. a Java runtime or a application server.
...
example plugin configuration for a +war+ project can be found in <<yum-rpm-pom>>.
