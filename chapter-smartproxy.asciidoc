...
On the publishing repository manager you have to enable smart proxy on the
...
In addition you can find one smart proxy capability for each repository 
configured to be publish or subscribe updates with Smart Proxy.
...
TIP: A series of videos demonstrating Smart Proxy is available http://www.sonatype.org/nexus/members-only/video-gallery-2/free-training-nexus-professional-and-smart-proxy/[on the Nexus community site].
////
/* Local Variables: */
