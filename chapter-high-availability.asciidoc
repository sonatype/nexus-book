...
HA-LC, and procedures for setting it up.
...
complex, as is the process for upgrading to newer versions of the {rm}.

HA-LC is not intended to improve the throughput or performance of a {rm} deployment, nor is it a replacement for
...
Internally, the {rm} uses http://orientdb.com/orientdb/[OrientDB], a clustered database. Orient replicates state
...
limited to) the {rm} configuration properties, JVM settings, Jetty configuration overrides, and Hazelcast configuration.

In addition, the {rm} does not replicate the binary artifacts themselves, as these are stored in file system-based blob
