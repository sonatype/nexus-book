...
standard for automating system failover and implementing continuous uptime. This relieves teams of performing
...
share access to components. To configure:

1. Go to 'Blob stores'
2. Enter name into the field
3. Save.
...
When you enable high availability, the nodes discover on another via link:https://hazelcast.com/[Hazelcast].
...
4. Start all three repository managers.

[[high-availability-startup]]
==== Confirming Node Connectivity

When you start the nodes, you will see a message that confirms the connection of the cluster members:
...
=== Configuring an Existing Cluster after Setup

Once you have your high availability environment set up, be aware that almost all configuration done via the 
user interface is shared between all nodes in the cluster. There is no master node you must hit; they are all
treated equally. For example, if you create a new repository all nodes in the cluster will be able to see it
and utilize it. Or if you want to change your 'Email Server' port you just need to do it once via the user
interface on any of the servers and the change will share. Because all servers share out the changes,
changing on any is acceptable.
...
- Any configuration files you do (such as specifying a port via `nexus.properties` or setting up SSL)
