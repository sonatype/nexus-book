...
TBD (e.g. If you feel high availability is a solution your organization needs, consider the benefits:)
////
New section
////
...
All participating nodes require a unique port address in order to form the HA cluster. When unique hostnames are
assigned and high availabiity is enabled, synchronization occurs, as explained in  <<high-availability-system>>.
Since this is a requirement, learn the details about changing port address in <<config-http-port>>. 
