...
Bring the nodes to full service by starting the repository managers. As the console starts you can review the
log. You will see connection messages similar to this:
...
active data and active backup. Hazelcast can employ multicast to send data across a network, but it supports node
discovery in other ways.

By default, multicast is used to discover cluster members. If the default configuration isn't suitable for your
network infrastructure, you will need to customize `$install-dir/etc/fabric/hazelcast.xml`. See <<high-availability-aws>>
...
=== Configuring HA for Amazon Web Services
...
communication. If such a failure occurs you will need to modify the Hazelcast declaration file.
