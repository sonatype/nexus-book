...
between the nodes using distributed transactions. Each node votes on whether to accept a transaction - if a
...
Any deliberate changes to startup configuration need to be made on each node separately. This includes (but is not
limited to) {rm} configuration properties, JVM settings, Jetty configuration overrides, and Hazelcast configuration.

In addition, {rm} does not replicate binaries: these are stored in file system-based blob store. For an HA-LC
deployment, administrators must provide a highly available file system which is accessible to all three nodes.
