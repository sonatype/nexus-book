...
Each repository manager acts as a node and all nodes must share the same network configuration as the second
...
managers. You can install them across a mix of two:
...
When you have a workable file structure, where all three blob store file paths are the same, follow the steps 
...
- Download {pro}
- Duplicate the {pro} file, twice, creating a second and third instance of your repository manager
...
For example, let's say you create two nodes, a default called "node-a" and two new nodes called "node-b" and
"node-c", respectively. Then you designate the blob store directory for "node-a" at `sonatype-work/nexus3/blobs`.
You must mount "node-b" and "node-c" to the same location.
...
. Go to the second and third repository managers, and repeat steps 1 and 2, to enable them to use high availability, too.
