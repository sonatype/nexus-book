...
In some HA environments two-node cluster is minimum node redundancy. But, to reduce the probability of node
failure over a large-scale network we set the minimum count for a cluster to three nodes.

The appropriate quorum of nodes is an odd-number cluster, so we recommend a 3- or 5-node cluster before
recommending a 4-node cluster. Choosing an odd-number of nodes is optimal because if one crashes, your cluster
won’t fail the entire load over to a single node.

[[high-availability-os-support]]
=== Supported Systems for High Availability

There are multiple supported operating systems and platforms supporting node distribution in an HA cluster.
They may include:
...
Though it’s a requirement for your HA environment to share the same network, the mix of servers and platforms
may vary. Your network configuration may consist of the same operating systems or platforms, or perhaps even
different virtual operating system instances spread across different physical machines. Given the above, your
network isn’t limited to these scenarios. Whichever configuration you choose be mindful of server performance,
so your nodes can remain at their most optimal speed.
