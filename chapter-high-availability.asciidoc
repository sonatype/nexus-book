...
* Upgrade an older version of {pro} 3 to 3.3 that includes high availability.
...
availability, you can convert it into a cluster. Follow these steps:

1. Stop running the existing repository manager.
2. Follow the steps in the support article to upgrade the repository manager to version 3.3.
3. Reboot the repository manager in the newly upgraded version of the repository manager.
4. Enable high availability.
5. Download a second, then copy third instance to separate folders.
6. Start the second and third repository managers to form the cluster.
6. Set the location for the second and third repository managers in your file system so the nodes share same
the blob store.

Choose the node that want to be the parent, so that all configurations from the newly upgraded instance won't
get lost when you join the the second and third nodes needed to form the cluster.
