...
In {pro}, there are two ways you can implement high availability. In the scenarios below you will configure your
environment for redundancy. The cluster, which contains three redundant nodes, acts as one transparent instance.
...
5. Verify node synchronization from the administration menu.
...
When the cluster is established the second and third nodes are synchronized with the first. The nodes communicate
...
5. <<high-availability-nodes,Enable>> the first, newly upgraded node for high availability. Then enable
...
Regardless of the method you choose to enable HA, be  mindful that you shouldn't configure your nodes to share
an entire data directory.
...
At runtime, the repository manager user interface allows you to see the contents of one node synchronized with
the others. See <<nodes>> for details on viewing active nodes in cluster.
