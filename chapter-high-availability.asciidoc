...
active data and active backup. It uses multicast to discover HA-enabled nodes, and open commmunication among them.
So, instead of making manual adjustments to your servers' infrastructure, system configurations, and tools, Hazelcast
simplifies the configuration needed to maintain nodes in an HA cluster.

////
now, let's discuss how it work, and what to do when it fails
////
