...
The 'Members' from the terminal output represent the nodes that get discovered, automatically, at startup. The nodes
are synchronized via link:https://hazelcast.com/[Hazelcast], which provides in-memory computing for active data and
active backup.

Hazelcast relies on multicasting to discover HA-enabled nodes, and open communication among them. So, instead of
making manual adjustments to your servers' infrastructure, system configurations, and tools, Hazelcast simplifies the
configuration needed to maintain nodes in an HA cluster.
...
{nxrm} can be deployed on cloud-computing services, such as AWS. Depending on your network security, additional
configuration may be required. For example, if you use a network layer firewall application it may block multicast
communication. If such a failure occurs you will need to access `hazelcast.xml`. This configuration file can be
customized to circumvent failures preventing nodes to connect within a cluster. To configure Hazelcast for 
automatic discovery on secured networks, locate `hazelcast.xml` in `$install-dir/etc/fabric/`. Then:
