...
Joining three nodes is the minimum requirement for node redundancy. When you form a cluster, the three nodes act
as one transparent instance.
...
If you're working with a fresh installation of the repository manager, it comes with a 'default' blob store.
The default will not be shared with the additional nodes; therefore you need to configure the repositories
in the all nodes to point to new shared blob store location.
...
After completing these steps, you can create new repositories and point them to the new blob store configured
for shared access with a cluster.

==== Example: Configuring a Shared Blob Store for a Cluster

Let's say you create a new blob store called `data-location` and you want to point an 'npm (proxy)' repository
to this location, intended for shared blob store access within the cluster. You need to do the following:

1. Select a recipe from the 'Repositories' form, e.g.'npm (proxy)'
...
When you <<high-availability-nodes,enable>> the cluster, the components and assets of the primary node are
identically distributed to the second and third nodes.
