...
distributes traffic across a cluster of nodes, allowing horizontal scalability and reducing the need for a single
instance of {nxrm}.

This chapter guides you through the procedures to synchronize all repository data, system, and custom configurations
with the services provided by {nxrm}. Ultimately, the chapter explains how you can use {nxrm} as a continuously
operating, highly-available environment.
...
failure over a large-scale network we recommend the minimum count for a cluster to consist of three nodes.

The appropriate quorum of nodes is an odd-number cluster, so we recommend a 3 node cluster. Choosing an
odd-number of nodes is optimal because if one goes down, your cluster won’t fail the the entire load over to
a single node.
