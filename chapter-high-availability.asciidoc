...
1. Download a {pro} version of the repository manager supported with high availability.
2. Copy a minimum of two additional {pro} to establish three nodes, in different locations (e.g. three separate
directories).
...
If you have an earlier version of {pro} 3 with metadata and custom configurations that doesn't contain high
availability, you can convert it into a cluster. If you want to configure your non-HA-supported version so it's
synchronized with the second and third node, follow these steps:

1. Stop running the existing repository manager.
2. Follow the steps in the support article to upgrade the repository manager to a version that supports high
availability. Store the repository manager in its own directory.
3. Copy a second instance of the repository manager, and store it in its own separate directory.
4. Create a third instance by copying the second repository manager to its own separate directory.
5. <<high-availability-configure,Enable>> the first, newly upgraded node for high availability.
6. Enable high availability for the second and third nodes.
6. Configure a location for all three nodes to share the same the blob store.
7. Start all three nodes to form the cluster.
