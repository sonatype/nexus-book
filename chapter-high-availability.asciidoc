...
high availability. Store the repository manager in its own directory.
3. Download and unpackage a second repository manager, and store it in its own separate directory.
...
7. Configure a location for all three nodes to share the same the blob store.
8. Start all three nodes to form the cluster.
////
