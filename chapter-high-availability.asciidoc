...
This chapter guides you through the a solution to synchronize all repository data, system, and custom
configurations with the services provided by {nxrm}. Ultimately, the chapter explains how {nxrm} provides
all appropriate steps to create an highly availability environment.
...
This is an example of {nxrm} HA's key layers. The depiction shows the discovery of enabled nodes via Hazelcast
...
As stated in <<high-availability-storage>>, you must to provide your own shared memory and network configuration
to host shared data, such as blob stores and exported database content.
...
for details on what's need to properly configure an individual node for HA.
...
a new external location within it, and then then configure all repositories and data in that node to point to the
