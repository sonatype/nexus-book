...
Sonatype {oss} as well Sonatype {pro} are
...
by the availability of {oss} under the Eclipse Public
License, the REST API of Nexus and the support for plugins as part of
...
{pro} Users::: General discussion and support for
...
Sonatype, projects run by {pro} customers or {oss} users 
to one man, one-off hacks for some older version of Nexus.
...
All of the projects listed are community
...
jenksing plugin to add custom metadata with deployments to {pro}.
