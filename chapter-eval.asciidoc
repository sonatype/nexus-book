...
* *Step 2:* Run the {pro} Installer.

* *Step 3:* Start Nexus from the {pro} Installer.
...
to demonstrate the features of {oss} and {pro}. 
...
When you downloaded the trial distribution of {pro}, your
...
. Go to the Nexus evaluation guide directory you configured during the
  {pro} install and configure Maven to access Nexus
...
.Conclusion
**** 
Sonatype {oss} and Pro can serve as an important
...
The User Token feature of {pro} allows you to replace the
...
developers can connect to {pro} using the Nexus
...
Your {pro} Trial instance has been preconfigured with the
...
{pro} can improve collaboration and control, while
...
these components to a hosted NuGet repository on {pro}. 
...
Once NuGet packages are published to your {pro} instance
...
.Conclusion
****
Once NuGet packages are published to your {pro} instance and are
...
that have been merged during an acquisition. {pro}’s
...
.Conclusion
**** 
When you need LDAP integration, you will benefit from using {pro}. 
 {pro} can support the largest development
...
If your organization uses Atlassian Crowd, {pro} can
...
If you’ve consolidated authentication and access control using
 ...
 with it as well. {pro}’s support for Crowd makes this
 easy.
****
...
With the {pro} feature Smart Proxy, two distributed teams
