...
Apache Ivy can be installed manually if desired. However the example
projects used in this guide, contain ant targets in their build files
that will automatically install Apache Ivy as part of the build.
...
for Apache Maven, Gradle and Apache Ant/Apache Ivy based builds.
...
[[eval-ant-ivy-proxy]]
==== Proxying Components With Apache Ant/Apache Ivy
...
* Configure Ant to download components from Nexus
...
. Check out the lib folder in the projec after a build to see all the
libraries that got downloaded

. After the build has successfully completed, delete the local Gradle 
  cache and re-run the build and the lib folder.
+
----
...
==== Publishing Components With Apache Ant/Apache Ivy
...
. Follow the proxying evaluation example from <<eval-ant-ivy-proxy>>
...
. Observe how Ant logs the upload
+
----
[ivy:publish] :: publishing :: org.sonatype.nexus.trial#simple-project
[ivy:publish] 	published simple-project to http://localhost:8081/nexus/content/repositories/snapshots/org/sonatype/nexus/trial/simple-project/1.0-SNAPSHOT/simple-project-1.0-SNAPSHOT.jar
----
...
. This second project has a dependency on the first
  project declared in ivy.xml
+
----
<dependencies>
  <dependency org="org.sonatype.nexus.trial" name="simple-project"
      rev="1.0.0-SNAPSHOT"/>
</dependencies>
----
+
