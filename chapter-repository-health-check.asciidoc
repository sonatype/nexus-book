...
release policy configured. Availability of component meta data varies depending on the format and is constantly improved via updates to the Sonatype Data Services. 
...
component data is supplied by the Sonatype hosted data service to {pro} daily. The recurrence frequency can be
...
by the Sonatype data services. The next columns for the GAV parameters allow you to
