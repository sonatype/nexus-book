...
Repository health check can be setup for any repository, as long as ...

* The Repository 'Type' is 'Proxy'
* The Repository 'Policy' is *note* 'Snapshot'

For a single repository can be enabled in one of two ways. The quickest way is
to simply click the 'Analyze' button'. Alternatively, you can select the
repository in the list of repositories and then set the 'Enabled' configuration
in the 'Health Check' tab to 'true' as displayed in <<fig-rhc-enabled>>.
