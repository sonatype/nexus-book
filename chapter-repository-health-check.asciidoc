...
Repository health check can be setup for any repository, as long as it is...

* The Repository 'Type' is 'Proxy'
* The Repository 'Policy' is 'Release'

For a single repository can be enabled in one of two ways. The quickest way is to simply click the  'Analyze' button'. Alternatively, you can 
select the repository in the list of repositories and then set the
'Enabled' configuration in the 'Health Check' tab to 'true' as displayed in
<<fig-rhc-enabled>>. Administrator privileges are required to perform this
configuration.
...
presented with an acceptance of the Terms of Server, as well as an option to
analyze all compatible repositories, or just the currently selected one.
...
NOTE: If disabling this global analysis, be sure to refresh nexus prior to
