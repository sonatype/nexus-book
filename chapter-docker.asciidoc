...
=== Testing Docker Repository Availability

In order to test that a hosted or proxy Docker repository or repository group is up and running you can ping the API
with. E.g. with the name for a repository group of `docker-all` use

----
$ curl -X GET -v -k 'https://localhost:8443/repository/docker-all/v1/_ping'
----

This should return a HTTP OK (200) response:

----
....
> GET /repository/docker-all/v1/_ping HTTP/1.1
...
* SSLv2, Unknown (23):
< HTTP/1.1 200 OK
< Date: Thu, 10 Sep 2015 19:57:38 GMT
< Server: Nexus/3.0.0-b2015090801 (OSS)
...
[[docker-access]]
=== Accessing Docker Container Images
...
By default this search uses Docker Hub as preconfigured in Docker and will only find images available there

A more powerful search is provided by Nexus when searching against a Docker repository group with the syntax
...
An example looking for a `postgres` image on a Nexus server running on the host `repo.example.com` and exposing a Docker
repository group with a Docker connector port of 18443 looks looks like this:

----
$ docker search repo.example.com:18443/
----
