...
TIP: Docker is a fast moving project and requires usage of current operating system versions. For example, usage
of Red Hat Enterprise Linux 6 is simply not supported. Please use the https://docs.docker.com/[official
documentation] as additional help, for your usage.
...
validity of the certificate, by checking with certificate authorities. If you are using a purchased certificate,
...
certificate. Using a sef-signed certificate requires, further configuration steps to ensure that Docker accepts
the certificates and provides full functionality.

TIP: SSL certificates are not very expensive and we recommend buying a certicate for your {pro} to simplify Docker
usage.

Docker can be configured to skip certificate validation, which effectively allows usage self-signed certificates,
with the `--insecure-registry` flag. Each repository on the repository manager accessed from Docker, has to be
configured. An example set for docker accessing a {pro} running on localhost with a hosted repository on port
18444 and a repository group on port 18443 is:

----
--insecure-registry localhost:18843 --insecure-registry localhost:18444
----

The specific syntax of the start for the Docker daemon changed between versions of Docker. A manual start of for
...
If docker is configured to start up as a service on a Linux distribution, the configuration also varies based on
the distribtuion, its version as well as the startup system in use. For example on Ubuntu 15.10, a customized
startup can be configured in the file `/etc/systemd/system/docker.service.d/custom.conf`. The customized start can
be set up with:
