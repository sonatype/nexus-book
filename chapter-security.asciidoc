...
NuGet API-Key Realm:: This realm is required for deployments to NuGet repositories as documented in
<<nuget>>.
...
Select the 'Privilege Type' corresponding to the area of the repository manager you wish to grant permissions.
...
When you create a new 'Privilege Type' you can assign 'Actions'. 'Actions' are a list of functions allowing an
explicit behavior the privilege can perform with the associated function.

[[privileges-actions]]
==== Actions

The 'Actions' to choose from are  `add`, `browse`, `create`, `delete`, `edit`, `read`, `update`, and `*`. You
can assign a single or combination of comma-delimited actions when creating new privilege types. The roles to
which you apply any of these 'Actions' will perform the action's implied behavior. Consider how each action
behaves with an associated role, if the 'Privilege Type' contains any or all the actions below:


`add`:: This action allows the privilege type to add repositories or scripts.

`browse`:: This action is associated with privilege types that allow roles to browse the contents of associated
repositories. Though some privilege types associated with `browse` may only permit a given role to administrate
repositories but not see its contents. This action is distinct from `read` where an associated role may only be
able to look at contents only from UI.

`create`:: This action is associated with privilege types that allow roles to create such configurations as
blob store, user, and task creation.  Since a `read` permission is required to view a configuration, this
action is associated with most existing `create` actions.

`delete`:: This action allows roles with an associated privilege type to delete permissions for configurations
such as blob stores, capabilities, and LDAP. A `read` permission is associated with all existing `delete`
actions, which allows the privilege type to view a specific configuration that can be deleted. With this
action you can delete associated privilege types, such as 'Application', 'Repository Admin', 'Repository View',
and 'Script'. 

`edit`:: This action allows roles with an associated privilege type to make edits to a specific repository
in order to modify associated repository content.

`read`:: This actions allows roles with an associated privilege type to see permissions of various
configuration lists. Without this action, an associated action a role may see these lists but not its
contents. The `read` action also allows an associated privilege type to utilize tools that can look at
configuration contents from the command line.

`update`:: This action is associated with privilege types that allow roles to update such configurations as
blob store, user, and task creation.  Since a `read` permission is associated with a most update actions, a
privilege type that includes `update` can also view certain configurations in order to update them.
...
Search base:: The search base further qualifies the connection to the LDAP server. The search base usually
...
You can also reset the token of an individual user by selecting the 'User Token' tab in the 'Users' administration
from the 'Security' menu. The password requested for the action to proceed is the password for the authenticated
administrator who resets the token(s).
