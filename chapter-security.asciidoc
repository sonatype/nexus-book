...
The 'Proxy' configuration for each proxy repository documented in <<admin-repository-repositories>> a section titled
'Use the Nexus truststore' . It allows you to manage the SSL certificate of the remote repository and solves these
...
[[fig-ssl-certificate-details-dialog]]
.Certificate Details Dialog to Add an SSL to the Nexus Trust Store TBD-update screenshot
...
'Load certificate' button found there as described in <<ssl-certificates>>. If the certificate is already added, the
button can undo this operation and will read 'Remove certificate from trust store'.
...
Access <<fig-ssl-certificates-list>> by selecting the  'SSL Certificates' menu items in the 'Security' submenu in the
...
certificate in the 'Certifcate Details ???'  view.  The buttons are provided to 'Load from server' to add a new
certificate by loading it directly from a server or using a PEM file representing the certificate.
...
the remote host at the specified port. This allows you to load certificates from SMPT or LDAP servers, if you use the
correct port.
...
Nexus. TBD - bug .. it says button says "Load from Server..". An example of inserting such a certificate is shown in <<fig-ssl-pem>>.

[[fig-ssl-pem]]
.Providing a Certificate in PEM Format - TBD update screenshot
...
[[fig-ssl-add-server]]
.Certificate Details Displayed after Successful Retrieval - TBD update screenshot or remove
...
'custom.properties .. TBD really?' to set the system properties necessary to load this file. Make sure to adapt the
property numbers (10, 11) to start at the last unused value, which depends on the rest of your configuration.

----
TBD .. is this right for Nexus 3 ?
...
Let's elaborate on the second approach, which is to use the Eclipse Jetty instance that is distributed with Nexus
...
To configure the Nexus to accept HTTPS connections, first enable the file +jetty-https.xml+ to the
Jetty startup configuration in +custom.properties+ by adding 
...
property in +etc/org.sonatype.nexus.cfg+.
...
file suggests to create a subdirectory +NEXUS_HOME/conf/ssl+ and copy the +keystore+ file in there and rename it to
+keystore.jks+. You can either do that or choose a different location or filename for your keystore file and update the
paths for the +keystore+ and +truststore+ in the +jetty-https.xml+ file.
...
of `jetty-http-redirect-to-https.xml` as additional app parameters in +custom.properties+ as well as update or add the
`Base URL` in your Nexus server configuration using the <<admin-system-capabilities, Base URL capability>>.
