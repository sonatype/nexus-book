...
Privileges can be assigned to a role in the {pro} and {oss} users. When you assign a privilege type you can 
create, read, update, delete, edit, and browse. To start the process click 'Privileges' in the left sidebar, 
nested under 'Security'. In <<fig-privileges-list>> the 'Privileges' panel shows a list of built-in permissions 
your can assign to a role and, ultimately, apply to a user.
...
Click the 'Create privilege' button to view a list of privilege types you can customize for your roles, as seen 
in <<fig-privileges-types>>.
...
Application:: privileges related to the defined application.
...
NOTE: Some CRUD will work and some BREAD will work
...
form is completed for a privilege only allowed read access to an LDAP domain.
...
privileges for a Docker hosted repository.
