...
You can assign default or custom privileges to a role within the repository manager. Click 'Privileges' in the 
left sidebar, nested under 'Security' to access the screen. In <<fig-privileges-list>> the 'Privileges' panel shows a list of built-in permissions your can assign to a role and, ultimately, apply to a user.

[[fig-privileges-list]]
.List of Security Privileges
image::figs/web/privileges-list.png[scale=80]

The list of privileges displays an icon for the privilege Type as the first column and then adds:
...
Click the 'Create privilege' button to view a list of privilege types you can customize for your roles. This will 
take you to a screen that lists a number of types of privileges, seen in <<fig-privileges-types>>.

[[fig-privileges-types]]
.Choosing Privilege Types
image::figs/web/privileges-types.png[scale=45]

Select the privilege you want to apply to the role by clicking on the type. The privilege types are as follows:

Application:: privileges related to the user interface and related functionality.
Repository Admin:: privileges related to the administration and configuration of a specific repository; actions 
can be defined with  '*' (all), 'add', 'browse', 'delete', 'edit' and 'read'.
Repository View:: privileges controlling access to the content of a specific repository; actions 
can be defined with '*' (all), 'add', 'browse', 'delete', 'edit' and 'read' functions.
Script:: privileges related to scripted commands capable of '*' (all), 'add', 'browse', 'delete', 'edit' and 
'read' functions.
Wildcard:: privileges related to search that will match any sequence of permissions.

Below are some sample privileges you may assign to roles. The examples will show 'Application' and 
'Repository View' privilege types. 

In this example, create an Application privilege type.
...
form is completed for a role only allowed read access to an LDAP domain.

In another example, create a Repository View privilege type.
...
<<fig-privileges-repository-view>>, above, the form is completed for a role allowed edit privileges for a Docker 
hosted repository.

Click the 'Create privilege' button to save a new custom privilege. The privilege can be found listed among the 
default privileges on the main 'Privileges' screen.
