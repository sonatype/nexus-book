...
administration from the 'Security' menu. To assign the appropriate privileges so the user can reset the user
token:
...
3. Assign 'nx-usertoken-user', 'nx-users-all', and 'nx-roles-all' to the role.
4. Save the changes.

After login the user can reset the user token from the following steps:

1. Click 'Users' from limited 'Security' screen.
2. Choose the user with the assigned privileges, from the selection panel.
3. Click the 'User Token' button from the 'Users' screen.
4. Click 'Reset user token' to access the 'Authenticate' dialog.
5. Enter the password assigned to the user.
6. Click 'Authenticate' to validate your credentials, completing the reset.

The password requested for the action to proceed is the password for the authenticated administrator who resets
the token(s).
