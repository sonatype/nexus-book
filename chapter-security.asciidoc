...
. Edit `$data-dir/etc/nexus.properties`. Change the `nexus-args` property comma delimited value to include 
  `${jetty.etc}/jetty-https.xml`. Uncomment this line if currently commented. Save the file.

. Restart Nexus. Verify HTTPS connections can be established.

. If using it, update the `Base URL` to use `https` in your repository manager configuration using the
