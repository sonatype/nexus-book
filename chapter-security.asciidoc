...
The 'User Token' dialog displays a user code and pass code tokens in separate fields. Below the token, is your 
Maven `settings.xml` server file. When using the server section you simply have to replace the `${server}` 
placeholder with the repository id that references your repository manager you want to authenticate against with 
the user token. The dialog will close automatically after one minute or simply click the 'Close' button.
