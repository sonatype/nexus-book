...
privileges for roles are already built in the repository manager, partially depicted in 
<<fig-privileges-partial-list>>. This feature also allows your to create custom privileges for roles within the 
repository manager.
...
Application:: These are privileges related to a defined aspect of an application.
...
Script:: These are privileges related to scripted commands capable of executing permissions granted.
...
either be exclusively or a combination of add, browse, create, delete, edit, read and * (all) storage 
functions.
...
Below are some sample privileges you can create. The examples will show 'Application' and 
