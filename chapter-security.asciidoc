...
preferences. The file lives in +$HOME/.m2+, your main Maven directory. This file contains listings for 
personalized client or build-tool configurations such as repositories. This file is not exclusive to 
Maven-specific repositories.

The general setup and use is cumbersome, and the potential need for regular changes due to strong security 
requirements e.g., with regular, required password changes triggers the need for a simpler, more secure solution. 
The Maven framework has the ability to encrypt passwords within the +setting.xml+, but the need for it to be 
reversible in order to be used, limits its security.
...
privilege. If given that privilege click the 'User Token' menu item under 'Security' in the 'Administration' 
menu. Check the 'Enable user tokens' box, then press 'Save' to activate the feature. You can access the user 
token from the 'Profile' mode toggle, explained in <<access-token>>.
...
With enabled, you can access your individual token on your 'Account' panel. To access the panel, select the 
'admin' mode toggle, on the top right-hand corner of the user interface.
...
The 'User Token' dialog displays user code and pass code tokens in separate fields. Below the token, is your 
