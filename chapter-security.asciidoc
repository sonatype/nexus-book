...
You can also assign privileges to users and any assigned role, so they can access a specific group repository.
This allows the user, and applicable roles, to access any content within any member repository in that group.
However, this level of security only permits users (and applicable roles) direct access to a member repository
assigned to that group. This privilege is limited to read-only (`read`) access.
