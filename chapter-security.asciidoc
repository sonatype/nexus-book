...
In order to use LDAP authentication in the repository manager, you need to load 'LDAP Realm' to Active realms. To 
do the load click 'LDAP Realm' in the Available bin and move it to the Active bin. As shown in
<<fig-active-realm>>, 'LDAP Realm' is listed below 'Nexus Authentication Realm' and 'Nexus Authorizing Realm'.
...
<<fig-security-ldap-user-group-temp-config>> that will adjust the rest of the field inputs based on your 
template selection.

[[fig-security-ldap-user-group-temp-config]]
