...
systems - dynamic or static. A dynamic group is a list of groups to which users belong. A static group contains a 
list of users.
...
'uniqueMember' attributes. In a dynamic group configuration, each user entry in LDAP contains an attribute that 
lists group membership.
