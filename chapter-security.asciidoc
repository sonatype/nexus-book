...
Privileges can be assigned to a role for both {pro} and {oss} users.

To start the process click 'Privileges' in the left sidebar, nested under 'Security'. In <<fig-privileges-list>> 
the 'Privileges' panel shows a list of built-in permissions you can assign to a role and, ultimately, apply to a 
user.

[[fig-privileges-list]]
.Partial List of Security Privileges
image::figs/web/privileges-list.png[scale=35]
...
image::figs/web/privileges-types.png[scale=35]
...
Application:: privileges related to a defined aspect of an application.
Repository Admin:: privileges related to the administration and configuration of a specific repository.
Repository View:: privileges controlling access to the content of a specific repository.
Script:: privileges related to scripted commands capable of executing permissions granted.
Wildcard:: privileges related to search that can match any sequence of permissions.

In all 'Privilege Types', above, the variables assigned to a role are defined as 'Actions'. 'Actions' can 
either be CRUD (create, read, update, and delele) or BREAD (browse, read, edit, add, and delete) storage 
functions.
...
image::figs/web/privileges-application.png[scale=35]
...
image::figs/web/privileges-repository-view.png[scale=35]
