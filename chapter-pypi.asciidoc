...
Alternately you can create a +setup.cfg+. Like +pip.conf+, the +index-url+ is the tag created to specify the base 
URL for the PyPI proxy index.
...
////
In paragraph below, add section that consumers can download the registered package via the repository group
...
If you use pip to maintain your packages from the index, you can manage inbound and outbound communication 
between the repository manager and the PyPI repository with a self-signed SSL certificate. This assumes the 
repository manager has already been set up to use SSL.
