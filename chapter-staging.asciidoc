...
The Staging Suite is part of the default {pro} install
...
of rules which will control the way in that Nexus intercepts a
...
staging profile identifier and to fall back to an automatic determination, 
...
version of {pro} provides the option +Maven2 (hosted,
...
staging URL matching a repository target configured for a staging
...
staging repository has to be done via the Nexus user interface or the
...
Gradle allowing full integration of the staging suite features in a
Gradle build. 
...
{pro}+ is an important component that can
