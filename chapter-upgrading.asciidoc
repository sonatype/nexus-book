...
WARNING: Upgrading {nxrm} 2 to 3 provides tooling to transfer content and configurations from the respective source
repository manager to the target repository manager. These steps and procedures do not support large-scale data center
...
After the software has been upgraded on each node, the nodes can be brought back online one at a
time, allowing each node to startup completely before bringing the next node online.
...
NOTE:  Upgrading data and configurations while simultaneouly moving any dependent data across data centers is
prohibited. If performance issues occur, our Support team must be able to diagnose errors inherent to {nxrm}.
