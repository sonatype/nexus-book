...
There are equivalent configurations possible for all these values and customizations in {nxrm} 3. The specifics
vary widely and have to be applied manually after determining the need for such customizations and developing
specific plans for the modifications. The scope of these varies from zero to large efforts. E.g. some VM start-up
parameters might not be appropriate any more due to optimized performance of version 3. On the other hand custom
plugin features might now be a standard supported features.
...
configuration and data. This is the slowest optio.
...
With the above complete, you can use the upgrade tool in version 3, which guides you through
...
menu, 'Destination' gives your option to pick a blob store name different from default. The second drop-down
...
<<fig-upgrade-repositories-partial>>, you can select all repositories with one click, at the top of 
the table. Alternatively, you can click each individual repository. In addition to 'Repository', the table 
displays information about the status of the repository.
...
There are a few supported methods to choose from when running the upgrade. You can:
...
selected)
- transfer non-repository configurations, then return and upgrade all repositories
- transfer everything, abort at any stage, then re-initiate a second upgrade

////
The section above should include talking points on single upgrade and multiple upgrade efforts. Comments in the PR
////

==== After the Upgrade

With the upgrade completed and all components are transferred you can perform the next steps in your upgrade
