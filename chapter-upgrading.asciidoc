...
If HTTP or HTTPS proxy settings are enabled in your source {nxrm} 2 to target {nxrm} 3, the upgrade for this
configuration may fail because the target cannot communicate with the source repository manager. Instead,
version 3 will use its original settings, and a warning will be generated in the log if this error occurs.
