...
If HTTP or HTTPS proxy settings are enabled in your source {nxrm} 2, the upgrade to your target {nxrm} 3 may fail
because the target cannot communicate with the source repository manager. Instead, version 3 will use its original
HTTP or HTTPS settings, and a warning will be generated in the log if this error occurs.
