...
Agent Connection:: This screen presents two fields, 'URL' and 'Access Token'. Add the server's service
location from version 2 in the 'URL' field to establish the connection. The default working path for the 
enabled capability in version 2 is +/nexus/service/siesta/migrationagent+. Therefore an example input would be 
+http://localhost:8081/nexus/service/siesta/migrationagent+. In the second field, 'Access Token', add the 
