...
The 'Repository configuration and content' option creates {nxrm} 3 content selector privileges for repositories
that are explicitly transferred from {nxrm} 2. When subsets of repositories are transferred over multiple upgrades,
make sure your roles and the users tied to those roles are kept updated. The simplest way to do this is to always
upgrade user accounts from {nxrm} 2 when the 'Repository configuration and content' is selected.

If certain user accounts aren't desired in {nxrm} 3 then you can delete user accounts from the {nxrm} 3 after the
upgrade for all repositories is completed.
