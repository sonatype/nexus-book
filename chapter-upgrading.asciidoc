...
. Select 'Upgrade' located under 'System' to open the wizard, shown in <<fig-upgrade-wizard-intro>>.

[[fig-upgrade-wizard-intro]]
.Starting the Upgrade Wizard
image::figs/web/upgrade-wizard-intro.png[scale=70]
...
Content:: This screen, shown in <<fig-upgrade-content-options>>, allows you to select from compatible component 
formats ('Repositories'), security features ('Security'), and server configuration ('System'). For 'Repositories' 
you can select 'User-Managed Repositories', 'Repository Targets', and 'Health Check'. For 'Security' you can 
choose from 'Anonymous', 'LDAP Configuration', 'NuGet API-Key', 'Realms', 'Roles', 'SSL Certificates', 'Target 
Privileges', 'Users', 'Crowd', and 'User Tokens'. For 'System' you can select 'Email', 'HTTP Configuration', and 
'IQ Server' configurations.

[[fig-upgrade-content-options]]
.Selecting Content for Upgrade
image::figs/web/upgrade-content-options.png[scale=70]
...
.Selecting Content for Upgrade
image::figs/web/upgrade-repositories-partial.png[scale=70]
