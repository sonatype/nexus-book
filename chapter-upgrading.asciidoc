...
* custom log levels or edits to `logback.xml` configuration files (e.g. custom log file rotation, file naming, log
  patterns)
* Nexus Firewall (audit and quarantine) cannot currently be upgraded from {nxrm} 2 to {nxrm} 3 despite the fact 
  it is supported in both versions
...
enough file handlers available for the content you want to transfer during the upgrade process. On the other hand,
file copy and downloading do duplicate your data so upwards of double the original storage space can be, at least
temporarily, needed.
