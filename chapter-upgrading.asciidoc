...
Upgrading {nxrm} 2 to 3 only provides tooling to transfer content and configurations from the respective source
repository manager to the target repository manager. The upgrade agent provided by {nxrm} should not act as
a data center to data center transfer mechanism.
...
As mentioned in <<why-upgrade,Why Upgrade?>> upgrading data and configurations while simultaneouly moving any
dependent data across data centers is prohibited. In other words, your version 2-to-3 upgrade plan should be isolated
from other transfer procedures in your organization. If {nxrm} 2 to 3 upgrade performance issues occur, our Support
team must be able to diagnose such errors inherent to {nxrm}.
...
WARNING: As mentioned in <<upgrade-process-expectations,processes and expectations>> we recommended that you use
this upgrade guide solely to transfer content from {nxrm} 2 to 3. Please avoid using the upgrade tool to execute
cloud to on-premises data center transfers, or vice versa.
