...
Professional contains an optional security realm that allows you to
...
==== Installing Nexus Crowd Support

Starting with Nexus 2.7, Nexus Crowd support is already installed
and ready to configure.

In older Nexus versions, the Nexus Crowd support is implemented as
an otional plugin that comes as part of any Nexus Professional download.
...
[[crowd-sect-nx]]
==== Preparing Atlassian Crowd to Work with Nexus
...
with Administrative rights, and click on the Applications tab. Click on
...
Login as a user with Administrative privileges. To configure the Crowd
