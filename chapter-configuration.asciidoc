...
CAUTION: The deletion of Maven snapshots when a release occurs takes precedence over the number you select 
in the 'Minimum snapshot count' dropdown. If configured improperly, this task configuration may supplant the 
ability to delete snapshots associated with a release. Also, it is possible to configure the task in such a 
way that the results are unexpected. For example, if configured to keep 0 minimum snapshots older than 0 days, 
