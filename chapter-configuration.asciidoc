...
The `Base URL` is the endpoint at which users should access the user interface. This setting is used to host 
the repository manager behind a reverse proxy server. The basic requirement is to make sure the context path 
explicitly matches the base URL down to the root folder (e.g +/+, +/nexus+, or +/nexus3+). You do not need to set 
the base URL if you are not proxying the repository manager, because it knows its own default URL. 

Before setting your base URL, configure the hosted reverse proxy at <<reverse-proxy>>.

Creating or updating the base URL is a system capability. Go to the 'System' section of the 'Administration' menu 
and select 'Capabilities'. Click the 'Create capability' button and choose 'Base URL' from the 'Select Capability 
Type', then enter the server URL in the field. To save the entry press the 'Create capability' button.
...
Users with sufficient privileges can generate a record of user access during a session in the repository manager. 
Go to the 'License Users' feature view, nested below 'Licensing', to access the table. 

The table displays the IP address ('IP'), last entry date ('Date'), user privilege ('User'), e.g. 'admin', and 
client ('User agent'). To access the report click 'Download' to produce a CSV file of all users.
