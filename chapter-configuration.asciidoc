...
from a chosen blob store. When executed, the task searches the selected blob store for blobs missing their
associated metadata. The asset/components metadata is restored based on the information contained in the blob
store.
