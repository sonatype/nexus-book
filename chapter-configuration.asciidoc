...
'System':: The <<admin-system, general configuration of Nexus>> for
...
<<admin-system-http,'HTTP/S proxy server setting'>>
...
specific capability. This view includes the display of 'Type',
'Description', 'State' and 'Category' in the 'Summary' section as well
as further information in the 'Status', 'About' and 'Notes' section.
The 'Status' section displays a text message that details the status
of the capability and any potential problems with the configuration.
Depending on the capability, the reasons can vary widely.  The 'About'
section displays a descriptive text about the purpose of the
capability.  The 'Notes' field can be used to provide a descriptive
text about the capability or any other notes related to it and can be
persisted by pressing the 'Save' button.
...
plugin or advanced features of the smart proxy configuration are only
done in the capabilities administration

Usage of specific capabilities to achieve a variety of tasks is
documented in parts of the documentation.
...
Nexus supports proxy repositories, hosted repositories, virtual
...
Rebuild Metadata:: rebuild the metadata of a hosted Maven 2 repository
...
request, and set the timeout and retry behaviour. The HTTP request
