...
SSL/TLS options:: These checkboxes allow you to configure secure email settings for Nexus. 
The available options allow
you to enable or require STARTTLS (which upgrades the initially established, plain connection to be encrypted), enable
SSL/TLS encryption once a connection is made, or verify the identity of the server (using an identity check per RFC
2595). Finally, you may select the Nexus trust store for these operations.

Enable STARTTL support for insecure connections;; tbd
Require STARTTLS support;; tbd
Enable SSL/TLS encryption upon connection;; tbd
Enable server identity check (RFC 2595);; tbd
Use Nexus trust store;; tbd
