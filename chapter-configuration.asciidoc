...
deletion of the relevant files, and therefore frees up the storage space. On this form you are required to 
assign a 'Task name', 'Blob store', and 'Task frequency'. Optionally, you can assign a notification email.
...
including a developer's IDE, for example. The task publishes the index for all or a specific 'maven2'-formatted
repository.
...
Remove Snapshots from Maven Repository:: This task can be scheduled to remove Maven Snapshots. Often, you will 
want to remove snapshots from a snapshot repository to preserve storage space. This task supports this deletion 
for time stamped snapshots as created by Maven 3.x in a deployment repository. Note that configuring and
running this job is not enough to reclaim disk space. Files are not deleted by the 'Remove Snapshots' job. They 
are only moved into the trash folder. Therefore you will also need to configure a scheduled job to empty 
the trash folder. When you create a scheduled task to remove snapshots, you can specify the 'Repository/Group' to 
affect as well as:
...
'Remove if released';; The checked 'Remove if released' will dump all corresponding snapshot releases.

'Grace period after release (days)';; This can lead to builds that still reference the snapshot
dependency to fail. This grace period parameter allows you to specify a number of days to delay the deletion, 
giving the respective projects referencing the snapshot dependency time to upgrade to the release component or 
the next snapshot version.

CAUTION: When a release occurs the action of deleting Maven snapshots takes supercedes the number you select in 
the 'Minimum snapshot count' dropdown. If configured improperly, this task configuration may supplant the ability 
to delete of snapshots associated with a release. Also, it is possible to configure the task in such a way that 
the results are unexpected. For example, if configured to keep 0 minimum snapshots older than 0 days, all 
snapshots everywhere will be deleted, despite whether or not a grace period is configured for releases.
