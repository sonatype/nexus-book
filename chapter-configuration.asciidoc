...
can manage for single or multiple deployments of nodes, in table form. To view the status of your active node click
'Nodes' in the 'Administration' menu under 'System'. When you click a row, you get a detailed summary of the chosen
node.
...
maintenance. For example, you could run read-only mode to prevent your server from filling up disk space. Read-only
mode can also be useful when you re-organize your filesystem around blob stores to prepare for shared storage. The
use cases to activate read-only mode may vary. To enable it:

1. Click 'Nodes', under 'System'  in the 'Administration' menu
2. Click 'Enable read-only mode'

WARNING:: Anything that would require a change to system, database or blob store configuration will fail during
read-only mode. This includes requests for components from proxy repositories that have not yet been cached locally.
Additionally, you cannot run scheduled tasks in read-only mode.

////
image placeholder nodes-read-only-mode.png
////

As shown in the image above, the button becomes 'Disable read-only mode' when enabled. A banner appears
above the main toolbar, notifying you that read-only mode is activated. If you enable high availability, with
read-only mode applied to one node, all nodes in a cluster will be changed to read-only, too. Disabling read-only
mode, by clicking the 'Disable read-only mode' button returns the server to its original, writable state. Though
it may take a few seconds for the banner to acknowledge the change, the original state takes immediate effect.
