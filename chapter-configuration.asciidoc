...
TIP: `https://maven.oracle.com` is a server that requires 'Enable circular redirects'. When requesting data,
you are redirected to a queue of different URLs, most of which are involved with authentication. You can limit
the number of redirects, and increase performance, by checking the 'Enable cookies' box. This allows the repository
manager to maintain the authentication state in a cookie that would be sent with each request, eliminating the
need for the authentication-related redirects.
