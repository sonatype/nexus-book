...
A pattern that would capture more components like all components with any part of the path containing the format 
'maven2' could be defined as 'format == 'maven''.
...
can assign a role as documented in <<roles>>. When roles are defined, assign a user, aa mentiones in <<users>>.
...
Below are the allowable attributes for content selectors. In this table the 'path', 'format', and 'coordinate' 
variables for formats supported by {nxrm}, and their corresponding ""
