...
URL:: the 'copy' button prompts a dialog with a direct URL path exposing the repository via HTTP. Depending on the
repository format the URL provides access to the corresponding repository and directory browsing.

Health Check:: the result counts for a <<rhc,repository health check>> with a button to start the analysis
