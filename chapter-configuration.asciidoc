...
configure the connection to a proxy server for HTTP and a potentially a different proxy server for HTTPS connections. If
you do not configure a proxy for HTTPS, the HTTP proxy server settings will be used. To configure a HTTP proxy, select
the checkbox beside 'HTTP Proxy' and configure the parameters in the sections displayed in
...
In addition, you can configure a number of hosts that can be reached directly. Requests to them should not go through
the configured HTTP/HTTPS proxy. These hosts can be configured in the 'Hosts to exclude from HTTP/HTTPS proxy'
setting. You can add a hostname in the input box and add it with the `+` button. The `*` character can be used for
wildcard matching for numerous host names allowing a setting such as `*.example.com`. Entries can be removed with the
`x` button.
...
+$NEXUS_HOME/data/etc/logback-overrides.xml+ and override any
...
`$NEXUS_HOME/data/downloads` and opens a dialog to download
...
+TBD/nexus/conf/branding.png+. The new logo needs to be a
...
# Nexus section
nexus-work=${bundleBasedir}/../TBD/nexus
