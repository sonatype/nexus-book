...
expression library used to script queries along specific paths and coordinates available to your 
...
define, in a simplified example, a selector named Apache Maven with a search expression of 'path =^ "/org/apache/maven/*"'. This would match all components within the designated component path. You can refer to 
...
in 'Repository', from the 'Administration' menu. Click 'Create selector' to open a new form. In the 'Selector ID' 
section enter a 'Name' and (optional) 'Description' of your selector in the corresponding fields. In the 
'Specification' section use the 'Search expression' field to build your query using JEXL syntax. All saved 
selector queries you create will be listed in the 'Content Selectors' screen.

////
Add a section to talk about the selector preview feature in https://issues.sonatype.org/browse/NEXUS-10377
////
...
(all), for components matching that path. The privilege can even span multiple repositories.
