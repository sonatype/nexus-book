...
If using the high availability service, all synchronized servers are displayed on the 'Nodes' screen. View the table
to monitor and verify server connections.
...
Database backup:: This scheduled task performs a full system recovery of your OrientDB database, including logs,
repository manager configurations, and security configurations. You can name and choose a file location for this
task. When you run the backup, the repository manager creates a timestamp of the latest backup to your storage
destination.
