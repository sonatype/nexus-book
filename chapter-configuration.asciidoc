...
field - Remote User Transfer 'rut' authentication. Typically this is done
...
<<confignx-sect-capabilities>> and setting the 'HTTP Header name' to the
header populated by your security system. Typically this value is
+REMOTE_USER+, but any arbitrary value can be set.
...
A very seamless integration can be set up for users if the external
