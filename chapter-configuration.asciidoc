...
The 'Nodes' screen provides a summary of active servers, particularly for the use of the high availability
service. When you start your repository manager, click 'Nodes' in the 'Administration' menu under 'System'.
You will see a table with attributes of the node you started. When you click a row, you get a detailed summary
of that node. The summary lists:
...
If using the high availability service, all synchronized servers are displayed on the 'Nodes' screen. View the
table to monitor and verify server connections.

The 'Nodes' interface also provides visibility when you want use a server as a cache during an upgrade to another
server. If you are in high availability mode, you run an upgrade without shutting down high availability cluster.
For example, if you have a cluster with three nodes, and you want to upgrade the cluster without completely shutting
the server down, switch the server to read-only mode by clicking the 'Enable read-only mode' button. With this mode
enabled you could upgrade one node from source at a time to the target server, while protecting the source database
from any new modifications that could disrupt the upgrade.
