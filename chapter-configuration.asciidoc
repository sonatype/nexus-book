...
Also, from the 'Nodes' screen you can protect your server's database from write access during an upgrade by
activating read-only mode. This mode can be useful if you need to disable database modifications when
synchronizing servers for high availability or running diagnostics. To enable it:

* click 'Nodes', under 'System'  in the 'Administration' menu
* click 'Enable read-only mode'

When enabled, the button becomes 'Disable read-only mode' and  banner appears above the main toolbar, notifying
your read-only is activated. Disabling read-only mode, by clicking the 'Disable read-only mode', returns the
server to its original, writable state.
