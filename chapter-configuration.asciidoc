...
Restore Asset/Component metadata from Blob Store;; This tasks allows you to recover lost component metadata
from a chosen blob store. When executed, the repository manager searches for missing binary parts associated with
the blob store you want to recover. The assets and/or components are restored to the state before loss.
