...
Content selectors is a set of regular expressions used to match characters on the path of components in a 
repository. Content selectors allow you to select specific content, with regular expressions that are defined by 
you from all our your content. The regular expressions are implemented with 
...
Content selectors allow you to define, for example, a selector called Apache Maven with a pattern of 
`^/org/apache/maven/.*`. This would match all components with the groupId `org.apache.maven` and any components 
within nested within the 'Group' such as `org.apache.maven.plugins`.

[[selector-create]]
===== Creating a New Content Selector

To create a new selector query, click 'Content Selectors' located in 'Administration' menu to get to the form. In 
the 'Selector ID' section enter a 'Name' and 'Description' (optional) of your selector in the corresponding 
