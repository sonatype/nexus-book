...
can manage for single or multiple deployments of node, in table form. To view the status of your active node click
...
view your local node (e.g. browser) will be listed as 'true'. Others node listed among the local node is 'false'.
View the table to monitor and verify server connections.
...
you that read-only mode is activated. Disabling read-only mode, by clicking the 'Disable read-only mode' button
returns the server to its original, writable state.
