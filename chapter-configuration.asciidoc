...
'User-agent customization';; Complete this field by appending a string to the user-agent HTTP header, e.g. adding
parameters to the requests in the URL parameters.
...
'Enable circular redirects';; Check the box to allow proxy repositories to continuously resubmit HTTP requests to
a remote server. For instance, proxying Oracle Maven requires circular redirects to prevent errors. Completing the
'Username' drop-down option in 'Authentication type' is required. See the https://support.sonatype.com/hc/en-us/articles/213465728-How-to-configure-a-proxy-repository-to-maven-oracle-com[support article] for further configuration instructions.
'Enable cookies';; Check the box to store HTTP requests from the remote server.
