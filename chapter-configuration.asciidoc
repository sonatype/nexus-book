...
Content selectors provide a means to select specific content, defined by you, from all of your content. You can 
create a content selector filter to match characters on the path of components in a repository. The content you 
select is evaluated against a https://commons.apache.org/proper/commons-jexl/[JEXL] expression. JEXL is a 
scripting library that provides the language for you to filter and query paths and coordinates available within 
your repository manager content.

Creating content selectors allow you to identify what selector privilege you can assign to a user. You can 
define, in a simplified example, a selector named Apache Maven with a search expression of `path =~ 
^/org/apache/maven/.*`. This would match all components within the designated component path. You can refer to 
more examples in <<selector-examples>>.

[[selector-create]]
===== Creating a New Content Selector

To identify a new selector privilege, create the query first. Click 'Content Selectors' located in 'Repository', 
from the 'Administration' menu. Alternately, click the 'Content Selectors' button located within the 'Repository' 
section. In the 'Selector ID' section enter a 'Name' and (optional) 'Description' of your selector in the 
corresponding fields. In the 'Specification' section use the 'Search expression' field to enter your JEXL query.
...
privilege that controls operations such as read, edit, delete, and '*' (all), for components matching that path. 
The privilege can even span multiple repositories.

As explained in <<privileges>>, create a new content selector privilege. Then choose 'Repository Content 
Selector' to get to a form that displays the following:
...
can assign a role as documented in <<roles>>. When roles are defined, assign a user, as mentioned in <<users>>.

A practical example might be where you delegate all control of components in `org.apache.commons` to a "Maven" 
...
This table contains a description of attributes that can affect the respective formats.
...
can define it utilizing the variable 'format'. You can use the syntax available in 
https://commons.apache.org/proper/commons-jexl/reference/syntax.html[Apache Commons] as a reference to create 
your queries.
...
*Selecting all 'maven2' content with a groupId that starts with 'org.apache.commons'*

'format == "maven2" && coordinate.groupId =^ 'org.apache.commons.''

*Selecting all 'nuget' package versions of EntityFramework*

'format == "nuget" && path =~ '/packages/EntityFramework/*.''
