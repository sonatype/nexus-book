...
* 'Enable circular redirects' - check the box to allow proxy repositories to resubmit HTTP requests to a remote
server. For instance, proxying Oracle Maven requires circular redirects to prevent errors. Completing the 'Username' in
'Authentication type' is required. See the https://support.sonatype.com/hc/en-us/articles/213465728-How-to-configure-a-proxy-repository-to-maven-oracle-com[support article] for further configuration instructions.
