...
maintenance. The use cases to activate read-only mode may vary. To enable it:
...
appears above the main toolbar, notifying you that read-only mode is activated.
...
Database backup:: This task performs a full back up of the underlying {nxrm} databases, including access
logs, repository manager configuration, and security configuration. You must choose a location for the backup data
for this task. When you run the backup, the task adds a timestamp to the backup files that are created in the
...
Purge unused docker manifests and images:: As new manifests are created and associated with a tag, previous 
manifests will still sit around in the repository providing no use.  This also applies to any images that are no 
longer referenced by a tagged manifest.  V1 layers that are no longer referenced by a tagged layer will also be 
removed.
