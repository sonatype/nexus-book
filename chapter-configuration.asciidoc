...
Xml Authenticating and Xml Authorizing Realm:: the Nexus internal
storage 

(Enterprise) LDAP Authentication Realm:: external storage in an LDAP
system with details documented in <<ldap>>

Crowd Realm:: external storage in Atlassian Crowd system with details
documented in <<crowd>>

Rut Auth Realm:: external authentication in any system with the user
authorization passed to Nexus in a HTTP  header field
...
Codehaus Snapshots:: This repository contains snapshot released from
Codehaus.
...
image::figs/web/configuring_auto-block.png[scale=65]
...
image::figs/web//repository-manager_add-to-group.png[scale=60]
...
image::figs/web/repository-manager_schedule-service.png[scale=25]
...
task can be used to drop all these repositories. You can configure the
duration of inactivity in days after which the repositories should be
dropped as well as the status of the repositories to include in the
check. Any change of the staging repository like a state change from
open to closed to promoted or released as well other changes to the
repository meta data like a description update are counted as an
activity. You can configure to scan open, closed and released
repositories for inactivity and therefore potentially drop them with
this task. This will allow you to avoid accumulating a large number of
stale staging repositories.
...
task tells Nexus to delete all proxied items which haven't been "used"
...
are try to conserve storage space and do not need all artifacts in the
future e.g. to reproduce old builds without renewed retrieval. This is
particularly useful for a personal Nexus deployment with a large
change rate of artifacts combined with limited diskspace.
...
image::figs/web/config-plugin-core-api-site.png[scale=30]

You can use the Nexus REST API to integrate Nexus in your external
systems. 

If you external integration uses Java you can use the Nexus client
using the dependency from <<fig-client-core-dependency>> with the
version corresponding to your Nexus server version.
...
'Security' sub-menu in the left-hand Nexus menu.
...
menu item in the 'Security' sub-menu in the left-hand Nexus menu.
...
image::figs/web/repository-manager_security-privileges.png[scale=50]
...
which is similar but not identical to the Perl syntax.
...
The built-in roles are managed by Nexus and can not be edited or
deleted. The role confirguration section below the list is visible,
but disabled for these roles.
...
these different access roles you e.g. fcan host your public and private
...
Trace' pane lists all of the privileges that have been granted to a
...
=== Request Access Logging in Nexus  
...
of these requests. E.g. you will be able to tell if the main load is
due to a CI server cluster or rather developers based on the IP
numbers of the requests as well as see the
spread of requests and load across different times. You also see what
URL's and therefore API calls and features are used in Nexus

Requests access logging is enabled by default in Nexus 2.8 or higher and uses a
performant and flexible LogBack implementation with built-in log
rotation already configured for 90 days of log file retention. The log
is written to the file +sonatype-work/nexus/logs/request.log+.

The configuration is located in +NEXUS_HOME/conf/logback-access.xml+
and can be changed to suit your requirments. If you change the file, a
...
Older versions of Nexus require different customization of the Jetty
...
Other build system use similar approaches and can benefit from the
...
usertoken-all by accessing the 'User Token' item in the 'Security' sub-menu on
...
http://shibboleth.net/[Shibboleth]. In many cases this achieved 
...
From a users perspective they are required to login into the
environment in a central login page, which then propagates the login
...
<<confignx-sect-capabilities>> and setting the 'HTTP Header name' to
...
name for configured authorization scheme. E.g. on a default Nexus
