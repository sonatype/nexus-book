...
Content selectors are a set of regular expressions used to match characters on the path of components in a 
repository. Content selectors allow you to select specific content, with regular expressions that are defined by 
you, from all of your content. The regular expressions are implemented with 
https://commons.apache.org/proper/commons-jexl/[JEXL], a scripting language, that allows filtering and querying 
on component patterns.

Content selectors allow you to define, in a simplified example, a selector named Apache Maven with a search 
expression of `^/org/apache/maven/.*`. This would match all components with the groupId `org.apache.maven` and 
any components within nested within the 'Group' such as `org.apache.maven.plugins`.

[[selector-create]]
===== Creating a New Content Selector

To create a new selector query, click 'Content Selectors' located in 'Repository', from the 
'Administration' menu. Alternately, click the 'Content Selectors' button located within the 'Repository' section. 
In the 'Selector ID' section enter a 'Name' and (optional) 'Description' of your selector in the corresponding 
fields. In the 'Specification' section use the 'Search expression' field to enter your JEXL query.

Creating a new content selector allows you to identify what components or assets you want your user to have 
permission to access.

[[selector-examples]]
===== Content Selector Examples

If, for example, you want to create a pattern that would capture all components with any part of the path 
containing the format 'maven2' you can define it in place of the variable 'format'. You can use the syntax 
available in https://commons.apache.org/proper/commons-jexl/reference/syntax.html[Apache Commons] as a reference 
to create your queries.

Other JEXL examples may include:

*Selecting all 'raw' format content*
format == 'raw'

*Selecting all 'maven2' content with a group that starts with 'org.sonatype.nexus'*
format == 'maven' and coodinate.group =^'org.sonatype.nexus'

*Selecting all 'nuget' content in the 'mycompany' namespace*
format == 'nuget' and path =^'/MyCompany'.


[[selector-security]]
===== Managing Security with Content Selectors

Once you have created a content selector, you can utilize it as part of your security setup. You can add a new 
privilege that relates to the target and controls the BREAD (Browse, Read, Edit, Add, and Delete) operations for 
components matching that path. The privilege can even span multiple repositories.

To create a new content selector privilege click 'Privileges' located in the 'Administration' menu, under 
'Security'. From the 'Privileges' screen click the 'Create privileges' button to view a list of privilege types. 
Click 'Repository Content Selector' to get to a form that displays the following:

* 'Name':: Create a name for the content selector privilege.
* 'Description':: Add a brief description for the privilege.
* 'Content Selector':: Use the dropdown to select from a list of selectors you created in <<selector-create>>.
* 'Repository':: Choose from a list repositories you created, as documented in <<admin-repositories>>.
* 'Actions':: Grant read, edit, delete, or '*' (all) privileges for user access control.
 
After you create content selector privileges, regulating what permissible data you want the user to access, you 
can assign a role as documented in <<roles>>. When roles are defined, assign a user, aa mentioned in <<users>>.

With this setup you can delegate all control of components in +org.apache.commons+ to a "Maven" team. This 
way, you will not need to create separate repositories for each logical division of your components.

[[selector-reference]]
===== Content Selector Reference

Below are the allowable attributes for content selectors. The table below defines 'path', 'format', and 
'coordinate' as values supported by {nxrm}.

[width="60%",frame="topbot",options="header,footer"]
|=====================================================================
|Attribute  |Allowed Values
|path       |The path under a repository where your content is exposed
|format     |The format of the content for which you query
|coordinate |A map of attributes that differ by content format
|=====================================================================

This table contains a list of formats containing variables for coordinate name, group name, and format version.

[width="60%",frame="topbot",options="header,footer"]
|===============================
|Format   |Coordinate Attributes
|Bower    |name, version
|Docker   |name, version
|Maven2   |group, name, version
|npm      |group, name, version
|NuGet    |name, version
|PyPI     |name, version
|Raw      |name
