...
a CRON expression)'. Apart from 'manula', all choices trigger display
...
users as explained in more details in <<concepts>>. Configuring
repositories is an essential part of your Nexus configuration, since
...
locally in Nexus acting as a cache. Subsequent requests for the same
component are then fulfilled from the local storage therefore
eliminating the network and time overhead of retrieving the component
...
policy'. Its setting controls how a hosted repository allows or
disallows component deployment. 
...
will not check a repository for changed components thay have already
...
Hosted repositories can be configured to validate any new components
to see if the MIME-type corresponds to the content of the file by
enabling the 'Strict Content Type Validation'.
...
to manage the SSL certificate of the remote repository. 

////
The 'View certificate' button .... TBD
...
////
Strict Content Type Validation:: see proxy repo
////

Not found cache enabled/Not found cache TTL:: If Nexus fails to locate
an component, it will cache this result for a given number of
minutes. In other words, if Nexus can't find an component in a remote
...
Authentication:: This section allows you to set a 'Username' and
'Password' for plain authentication or 'Username', 'Password',
'Windows NTLM hostname' and 'Windows NTLM domain' for 'Windows NTLM'
...
'Snapshot' for development components that end up in '-SNAPSHOT' in
...
retrieval can still use the '-SNAPSHOT' version string.

===== Additional Configuration for Repositories Using the NuGet Format

The NuGet repository format works via communicattion to the remote
repository via http://www.odata.org/[OData] queries. These queries are
triggered by requests from client tools to Nexus and are cached
internally to enhance performance. The parameters 'Query cache size'
and 'Query cache age' can be used to configure the size of the cache
as well as the rate at which queries expire and are re-run.
...
* For a Maven2 format repositry, confirm that the 'Version policy'
...
deleted. The role configuration section below the list is visible but 
