...
* <<admin-system-http,HTP/HTTPS proxy server configuration>>
...
proxy server you can configure the needed connections details in 'Windows NTKM hostname' and 'Windows NTLM domain'.

In addition, you can configure a number of hosts that can be reached directly and request to them do not need to go
through the proxy in the 'Hosts to exclude from HTTP/HTTPS proxy' setting. You can add a hostname in the input box
and add it with the `+` button. The `*` character can be used for wildcard matching for numerous host names allowing a
setting such as `*.example.com`.
