...
WARNING: Anything that would require a change to database, blob store, and system configurations will fail during
...
As shown in <<fig-nodes-read-only-mode>>, the button becomes 'Disable read-only mode' when enabled. A banner
appears above the main toolbar, notifying you that read-only mode is activated. If you enable high availability,
with read-only mode applied to one node, all nodes in a cluster will be changed to read-only, too.
...
writable state. Though it may take a few seconds for the banner to acknowledge the change, the original state takes
immediate effect.
