...
The `Base URL` is the URL via which users can access the repository manager. This setting must be activated if 
you want to host the repository manager behind a reverse proxy server. You do not need to set the base URL if 
you are not proxying the repository manager, because it knows its own default URL. 

To configure and run a proxy server, see <<install-proxy>>.
