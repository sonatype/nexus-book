...
IMPORTANT: You must be logged in as a user with sufficient privileges to manage licenses.

A paid license is necessary to upgrade {oss} to {pro}, and to keep {pro} paid features operational. Licenses are
special +.lic+ files that can be uploaded using the *System -> Licensing* section of the *Administration* menu in the main
menu section.

[[admin-license-upgrade]]
==== Upgrade {oss} to {pro}

For instructions on how to upgrade {oss} to {pro} for the first time, refer to <<install>>.

[[admin-license-renew]]
==== Upload a renewed license

TIP: To avoid interruption of {pro} services, be sure to upload a renewed license *before* the existing license expires.

In the 'System -> Licensing' section of the 'Adminsitration' menu, you should see a summary of the existing license information
and a *Select license...* button near the bottom of the page:

.License info screen
image::figs/web/admin-license-info.png[scale=40]

To upload a license,

* press the 'Select license...' button
* select the correct +.lic+ file in the file selection dialog, and press 'Open'
* press the 'Install license' button

At this point you will see a user authentication dialog,

.Re-authenticate to manage licenses
image::figs/web/admin-license-authenticate.png[scale=35]

Enter your password and press 'Authenticate'

Now the terms of service dialog appears,

.License Terms of Service agreement
image::figs/web/admin-license-tos.png[scale=35]

When you have reviewed the terms of service, press 'I agree'

If the license upload is successful, you will briefly see a success message in the upper-right portion of the screen,

.Successful license uplaod
image::figs/web/admin-license-success.png[scale=35]

[[admin-license-failures]]
==== Possible licensing failures

The license upload may fail for any of the following reasons:

License Certificate has expired:: image:figs/web/admin-license-fail-expired.png[scale=35] The license you uploaded is already expired.
Unable to verify license:: image:figs/web/admin-license-fail-verify.png[scale=35] The license file is not a valid license format.
Invalid licensing subject:: image:figs/web/admin-license-fail-subject.png[scale=35] The current product is not included in the license file.
...
submission to allow us to ensure ongoing development is well aligned with your needs. In addition, Sonatype appreciates
