...
and set the timeout and retry behavior. This section refers to the
...
which Nexus will search to locate a particular artifact. Note that the 
order here doesn't matter, the order searched is set by the order of the
repositories in the group repository's configuration.
