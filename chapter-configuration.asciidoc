...
running this job is not enough to reclaim disk space.  You will also need to configure a scheduled job to empty 
the trash folder. Files are not deleted by the 'Remove Snapshots' job. They are only moved into
...
+
'Remove if released';; The checked 'Remove if released' box causes snapshots to be deleted as soon 
as the scheduled task is executed. 

NOTE: Checking this box enabled and a released component with the same
