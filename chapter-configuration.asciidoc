...
+$data-dir/log/tasks+. The file name of each task log is the type followed by the full date and time the task started.
For example: +repository-maven.purge-unused-snapshots-20170618153235.log+.

Generally the output of the task will go to both the `nexus.log` and the specific task log, however some tasks will only
go to the task log, while others only go to the `nexus.log`.

For long running tasks, a periodic update will be logged back to the `nexus.log` every minute. For most jobs this will
appear as +---- Mark ----+; however some tasks have the ability to report actual progress so you may see a percentage
value.
...
The output of every task run will go to a separate log file. See <<task-logging>> for more information.
