...
create a selector to match characters on the path of components in a repository. The content you select is 
evaluated against a JEXL expression. JEXL is an expression library used to script queries along specific 
paths and coordinates available to your repository manager formats.
...
This table contains a description of attributes that can affect the respective formats. You can review some 
simple JEXL queries that call the listed attributes, against specific formats in <<selector-examples>>.
