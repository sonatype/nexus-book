...
'Connection retries';; Enter the total number or connection attempts after an initial timeout.
...
TIP: You can optimize performance for proxying special repositories with unique authentication, such as the
Oracle Maven Repository (`https://maven.oracle.com`). In the instance you proxy the Oracle Maven Repository check
the 'Enable circular redirects' box in 'HTTP request settings' to automate a sequence of requests to the remote
server. If you check the 'Enable cookies' box the server maintains the authenticated state of the proxy repository.
Be sure to utilize your credentials for `https://maven.oracle.com` to access the remote server.

The settings in this section are applied to all HTTP requests made from the repository manager to the remote
repository being proxied. Enabling these settings will override any general settings defined in
<<admin-system-http>>.
