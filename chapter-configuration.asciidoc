...
You can also proxy special repositories that require outbound authentication from the 'HTTP request settings'
section. For example, to proxy the Oracle Maven Repository (`https://maven.oracle.com`), utilize your
credentials to access Oracle's remote server.

In order to optimize performance for outbound requests when proxying the Oracle Maven Repository check:

* 'Enable circular redirects' to automate a cycle of requests to the remote server
* 'Enable cookies' to allow the server to maintain the authenticated state of the proxy repository

With circular redirects enabled, redirects to login URL of the remote server will be avoided.
