...
You can optimize performance for proxying special repositories with unique authentication, such as the Oracle Maven
Repository. For proxying the Oracle Maven Repository you will utilize your credentials for `https://maven.oracle.com`
to access the remote server. Then, from the 'HTTP request settings' section:

1. Select the 'Authentication type' as 'Username'.
2. Enter the 'Username' and 'Password' for `https://maven.oracle.com`.
3. Check the 'Enable cookies' box.
4. Save the updates.

The settings in 'HTTP request settings' are applied to all HTTP requests made from the repository manager to the
