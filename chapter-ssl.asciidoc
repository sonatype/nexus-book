...
+$(NEXUS_HOME)/bin/jsw/conf/wrapper.conf+ (in case you've customized
this location).
...
In order to ensure HTTPS is used for all connection an external proxy
can be used. Alternatively Nexus 2.7 users can use
+conf/examples/jetty-http-redirect-to-https.xml+ and Nexus 2.8+ users
can add usage of +conf/jetty-http-redirect-to-https.xml+ and
+conf/jetty-https.xml+ as additional app parameters in
+wrapper.conf+. 
