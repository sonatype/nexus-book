...
. Configure JSSE system properties for the {nxrm} process so that the custom truststore is consulted instead of the
  default file.
...
{nxrm} to accept HTTPS connections.
...
Instructions are available on the http://www.eclipse.org/jetty/documentation/current/configuring-ssl.html[Eclipse Jetty documentation site]. You may find the common keytool commands in 
