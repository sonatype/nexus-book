...
* add the file +jetty-https.xml+ to the Jetty startup configuration in
+wrapper.conf+ as detailed in <<jetty-xml>>. 

* set the port to be used by defining +application-port-ssl+ in
  +nexus.properties+

* configure the Java keystore and update the configuration in
  +jetty-https.xml+. The default configuration points to the keystore
  within Nexus, which is managed by the SSL feature documented in
  <<ssl-sect-client-cert-mgt>>. Alternatively you can point to an
  external keystore.

If desired you can configure automatic redirection from HTTP to HTTPS
with the by adding usage of +jetty-http-redirect-to-https.xml+ as
additional app parameters in +wrapper.conf+.
