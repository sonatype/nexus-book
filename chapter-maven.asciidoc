...
.Nexus Jenkins Plugin Upload
image::figs/web/jenkins-plugin-upload.png[scale=45]
+
...
.Nexus Jenkins Plugin Global Configuration
...
.Nexus Jenkins Plugin Tool Configuration
...
.. *Packages*: (optional) Select Packages to publish to {nxrm} during your Freestyle build. For this example, use the 'Add Package' dropdown to select a Maven Package. 
+
...
.Nexus Jenkins War in Release Repo
...
. Run the build. Once completed, you are presented with a successful build similar to what's shown below.
