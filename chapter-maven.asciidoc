...
----
/**
 * init.gradle file for development using the {nxrm} as proxy repository
 * 
...
Nexus for Jenkins is distributed as a Hudson plugin package (+.hpi+ file) and is available for link:http://download.sonatype.com/nexus/ci/latest.hpi[download from Sonatype Support]. 

To install Nexus for Jenkins, perform the following steps:

. Login to Jenkins as an administrator.
. Select 'Manage Jenkins' from the navigation menu on the left.
. Select 'Manage Plugins' from the list of configuration options.
. Click the 'Advanced' tab on the 'Plugin Manager' screen.
. In the 'Upload Plugin' section click 'Choose File', open the downloaded Nexus for Jenkins hpi file, and then click 'Upload'. 

A message displays on the screen when Nexus for Jenkins is successfully installed.
...
. In the 'Sonatype Nexus' section, select '{nxrm} 2.x Server' from the 'Add {nxrm} Server' dropdown menu and then enter the following:
...
NOTE: The examples provided in this guide use Maven to build in a project. A Maven installation must be configured in Jenkins for these examples to work.
...
. Configure a Maven installation as shown in the figure below:

[[jenkins-for-ci-job-config]]
==== Job Configuration
...
. To start, create a Freestyle Project by clicking on the 'New Item' link on the Dashboard. Give the project a name and click 'OK' to continue with the configuration.
. Setup the Freestyle project as desired.
. In the 'Build' section of the freestyle project configuration screen, click the 'Add Build Step' dropdown button and then select '{nxrm} Publisher'. Enter the following parameters:
...
.. *Packages*: Select Packages to publish to {nxrm} during your Freestyle build. 
+
NOTE: For this example, use the 'Add Package' dropdown to select a Maven Package. Fill in the Group, Artifact, and Version fields as shown in the example below.
+
. Complete your freestyle build as desired and click 'Save'.
. Launch a build for your project. When the build is finished you are presented with output similar to the example below:

Returning to your {nxrm} instance, you will see the Jenkins war is now available in the Releases repository:

===== Pipeline Build Step Configuration

Jenkins Pipeline builds allow for precise control and configuration over your build process. 

. To start, create a Pipeline project by clicking on the 'New Item' link on the Dashboard. Give the project a name and click 'OK' to continue with the configuration.
. Setup the General, Build Triggers, and Advanced Project Options sections as desired.
+
NOTE: Pipeline jobs allow you to configure the job via a Groovy script. The script is editable in the Pipeline section. In this example we are using the sample 'Github + Maven' script.

[start=3]
. In the 'Pipeline' section, click the 'try sample Pipeline' dropdown, and then select 'Github + Maven'.
. Below the Results stage, add a new {nxrm} Publish stage to the build pipeline script.
+ 
TIP: The {nxrm} Publisher build step should occur after the build so that the binaries are available for upload. For best reporting during the build process, the publish step should exist in its own stage below the Results stage.
...
. r=Run the build. Once completed, you are presented with a successful build similar to what's shown below.
