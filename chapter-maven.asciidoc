...
. In the 'Upload Plugin' section, click 'Choose File', open the link:http://download.sonatype.com/nexus/ci/latest.hpi[hpi file], and then click 'Upload'. 

[[fig-nexus-jenkins-plugin-upload]]
.Nexus for Jenkins Upload
...
.Nexus for Jenkins Global Configuration
...
NOTE: Nexus for Jenkins can upload artifacts to a Maven repository without a Maven installation, however the examples provided in this guide use Maven to build in a project. A Maven installation must be configured in Jenkins for these examples to work.
