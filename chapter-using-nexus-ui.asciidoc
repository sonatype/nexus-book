...
the component identifiers 'Group', 'Name' and 'Version'. 'Most popular version' contains the version number 
...
NOTE: Certain component assets will remain after deletion. For example, while a Docker image can be deleted, the 
layers that make it up remain after its deletion.
