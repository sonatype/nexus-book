...
component identifiers 'Group', 'Name' and 'Version'. The 'Most popular version' contains the version number of 
the same component that is most popular in its usage. 'Popularity' shows a relative percentage of popularity to 
other versions of this component. This popularity data is provided by the {ds} based on requests from the 
Central Repository and other data and not available for all components. 'Age' shows the age of the component. 

None of the popularity or age data is filled in without {rhc} enabled.  For more details on {rhc} see <<rhc>>.
...
image::figs/orig/component-information.png[scale=35]
...
button allows you to remove an asset. The information itself is broken up into two sections, accessable by pills below the 'Delete' button.
