...
avoid an unsatisfactory user experience. Internet Explorer 8 works up to {nxrm} 2.8 and is not supported for newer
releases.
...
drop-down to log out, as well as navigate to the users profile. The header displays the version of {nxrm} and
...
in the Repository Health Check feature.
...
Exposures CVE list displayed in <<fig-clm-cve-jetty>>.
...
The Audit and Quarantine features use Nexus IQ policy management to protect your development environment from 
serving risky, unwanted components. You can enable these features to identify, prevent, and release such 
components that may compromise a proxy repository. 

See our
link:http://links.sonatype.com/products/clm/doc[{iq} documentation] to learn about releasing components from
quarantine and other features.
...
WARNING: Let me guess? You installed {nxrm}, ran to the search box, typed in the name of a group or a component,
...
repository health check or an external resource such as a security mailing list.
...
{nxrm} provides feeds that expose system events. You can browse these feeds by clicking on 'System Feeds' under the
...
'Support Tools' provides a collection of useful information for monitoring and analyzing your {nxrm}
...
Nexus:: details about the versions of {nxrm} and the installed plugins, install and work directory location,
