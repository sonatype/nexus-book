...
component that is most popular in its usage. This popularity data is provided by the {ds} based on
...
button allows you to remove an asset. The 'Info' section for each assets contains a number of attributes about the
specific asset.
...
repository has a 'Maven2' section with attributes for 'path', 'extension', 'baseVersion', 'groupId', 'artifactId'
and 'version'.
