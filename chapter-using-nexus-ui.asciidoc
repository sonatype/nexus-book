...
like 'Custom', 'Maven' and others from the 'Browse' main menu. The
common feature view for the search without results is displayed in
<<fig-search>>.  
...
displays the current criteria e.g., 'Keywords'. Beside the current
...
componenent allowing the author to classify it as belonging to a
specific organization or group or simply creating a namespace to
separate different components. Not all repository formats use the
notion of a group. Some repository formats simply use a different name
for the concept e.g., +org+ for Apache Ivy.
...
specific standard and are simply a string, which can affect the sort
order and produce unexpected results.

Checksum - MD5, SHA-1 or SHA-512::  A checksum value of a component
file generated by an MD5, SHA-1 or SHA-512 algorithm.

In addition there are critieria that can be used to search for
...
and Groovy Grape. In concept it is equivalent to 'Group'.
...
and +module+ for Groovy Grape.  In concept it is equivalent to 'Group'.
...
become visible in the component list, with an example displayed in
<<fig-search-results>>. The components are listed with their 'Name',
'Group', 'Version' and 'Format' and sorted alphabetically by
'Name'. The top of the list includes a paging navigation with controls
for the first, previous, next and last pages as well as a numeric page
input and a refresh button.
