...
Using the repository manager Nexus as well as the tools for software
supply chain automation from Nexus Lifecycle requires an understanding
of a few concepts and terms like 'Component', 'Repository' and
others. This chapter provides you with all the necessary background
and knowledge as well as an idea of a progression in your usage of
Nexus and Nexus Lifecyle.   
...
as part of your software application or required as part of your build
process. It can also be an entire application or a static resource
like an image without any dynamic behaviour.
...
Components can, in turn, be composed of multiple components
...
contexts that are included as part of the WAR component.
...
platforms that are used the development of applications every day and
it has become a default pattern to build applications by combining the
features of muliple components with your own custom application code
to create an application for a specific domain.

In order to ease the consumption and usage of components, they are
aggregated into collections of components. These are most of the time
called a repository and are available on the internet. On different
platforms terms such as registry are used for the same concept.
...
and a host of others. Components in these repositories are accessed by
...
expose the components in them to the client tools. This defines a
repository format. 
...
interactions, or different diretory structures with format specific
files for the metadata.

=== An Example - Maven Repository Format

Maven developers are familiar with the concept of a repository. In the
case of a repository using the Maven format, the primary type of
binary component is a JAR file containing Java bytecode.  Howeverv
there is no limit to what type of component can be stored in a Maven
repository. For example, one could just as easily deploy WAR or EAR
archives, source archives, Flash libraries and applications, or Ruby
libraries to a Maven repository.
...
are required by that component. The ability to automatically calculate
a project's dependencies and transitive dependencies is made possible
by the standard and structure set by the Maven repository format.
...
repository. When you download and install Maven without any
customization, Maven will retrieve components from the Central
Repository which serves millions of Maven users every single day. It
is the default, built-in repository using the Maven repository format
managed by Sonatype.

In addition to the Central, there are a number of major organizations,
such as Red Hat, Oracle or the Apache Software foundation, which
maintain separate, additional repositories. Best practice to
facilitate these available repositories is to install Nexus and use it
to proxy and cache the contents on your own network.
...
artifactId:: An 'artifactId' is an identifier for a software component. A
...
components and the default repository built into Apache Maven. It can
be easily used from other build tools as well. Statistics about the
size of the Central Repository are available at
