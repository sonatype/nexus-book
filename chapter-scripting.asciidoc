...
mentioned above. In the following section we are going to look at some further detailed examples.
...
The API for scripts is capable of a nubmer of task. This section provides examples for script writing, publishing
and executing. Since scripts are typically longer than a single line and creating them in a separate file in
the IDE is recommended, using a helper script that formats a `.groovy` file into a JSON file and submits it to the
repository manager can be convenient approach.
...
shell script. All scripts in this folder can be published and executed via the `provision.sh` file or individually:
...
dockerRepositories.groovy:: configures a proxy repository of DockerHub, a hosted repositoryt and a group
