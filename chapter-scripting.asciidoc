...
a hosted repository using the Maven format in the simplest usage simply requires a name.
...
mentioned above. In the following section we look at some further detailed examples.
...
The API for scripts is capable of a number of different task. This section provides examples for script writing, publishing
...
repository manager can be a convenient approach.
...
dockerRepositories.groovy:: configures a proxy repository of DockerHub, a hosted repository and a group
