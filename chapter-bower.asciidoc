...
A hosted Bower repository can be used to deploy your own as well as third-party components. You can register a 
Bower package to install components (such as libraries or frameworks) against a hosted repository in Nexus. This 
needs to be flexible to allow users to register against git repos
...
recipe recipe as documented in <<admin-repositories>>.
...
Bower groups can contain hosted or proxy bower repositories. Groups operate by order of selection. In other words, the first asset in a given group is accessed; and each subsequent group is accessed, accordingly. If there are two components of the same name, the first asset will be grabbed, and so on.
