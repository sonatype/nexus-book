...
`nexus` section to your `.bowerrc` file, as in this example:

----
{     
   "registry" : {
      "search" : [ "http://username:password@host:port/repository/bower-all" ]
   },
   "resolvers" : [ "bower-nexus3-resolver" ],
   "nexus" : {
      "username" : "myusername",
...
TIP: If your organization's policy prevents clear text passwords from being saved in local configuration files, 
{pro} supports obfuscation of those fields using User Tokens as described in <<user-token>>.
